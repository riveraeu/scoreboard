...
- **Polymarket comparison (game totals only)**: After Kalshi `totalMarkets` are parsed, `gamma-api.polymarket.com/markets?closed=false&limit=200` is fetched and filtered to O/U markets. Each market is matched to a Kalshi total by team keyword (`POLY_TEAM_KW` lookup in `api/[...path].js` ~line 1092) and threshold value (`kalshiThreshold − 0.5` = Polymarket O/U line). `outcomePrices[overIdx]` gives the implied probability (×100 = `polyPct`). Cached in Redis at `poly:totals:{date}` with 300s TTL; busted by `?bust=1`. Play output includes `polyPct`, `polyVol`, `bestVenue` ("kalshi"|"polymarket"), `bestEdge` (truePct − min(kalshi,poly)). UI: edge badge shows `bestEdge`; tooltip shows "Kalshi: Xc | Polymarket: Yc"; teal `[P]` pill appears when Polymarket wins. Market report totals table has a `Poly` column (teal = better than Kalshi, gray = worse/unavailable).
