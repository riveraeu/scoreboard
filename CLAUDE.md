...
- **SimScore** (max 10 — 5 stats × 2pts each; `qualified: teamTotalSimScore >= 8`):
  - MLB: teamRPG (>5.0→2, >4.0→1, ≤4.0→0), oppERA (>4.5→2, >3.5→1, ≤3.5→0), H2H HR% (≥80%→2, ≥60%→1, <60%→0, null(<3 games)→1 abstain), parkRF (>1.05→2, >1.00→1, else 0), O/U line (≥9.5→2, ≥7.5→1, <7.5→0)
...
- **SimScore** (max 10, edge gate only — 5 stats × 2pts each):
  ...
  - H2H hit rate (`hitterH2HHitRatePts`): rate vs tonight's pitcher from gamelog (H2H dates only). **Requires ≥ 8 games for 2pts** (≥80% hit rate); 5–7 games caps at 1pt max (≥70% → 1pt, else 0pt); < 5 games → null → 1pt abstain. When platoon fallback active (`_hrrUsingTeamFallback = true`): uses platoon tier — `hitterPlatoonPts === 0` (disadvantage) → 0pts, else → 1pt abstain. **No team fallback for SimScore** — pitcher H2H only.
...
- **Team totals table** (`mlb|teamRuns`, `nba|teamPoints`): section header shows **"MLB Team Runs"** / **"NBA Team Points"** via `STAT_NAME` entries. First column labelled **"Team"** (shows `scoringTeam` only, clickable → team page); last XCOLS column **"Opp"** (`ttOpp` key) shows `oppTeam` as clickable span. Default sort: Score descending (seeded in `reportSort` initial state). Score column uses `m.teamTotalSimScore` (qual gate = 8); hover tooltip shows per-component breakdown. XCOLS: MLB = Score / Team RPG / Opp ERA / **H2H HR%** / Park / O/U / Opp; NBA = Score / Team PPG / Opp Def / O/U / Pace / **H2H HR%** / Opp. **H2H HR%** (`ttH2HHR` key): scoring team's hit rate ≥ threshold in last 10 H2H games vs opp (≥80% green, ≥60% yellow, <60% red; null=DASH; game count shown as hover title). MLB replaces Opp RPG; NBA replaces Spread. **Colors** match SimScore tiers: RPG >5.0 green / >4.0 yellow; ERA >4.5 green / >3.5 yellow; Park >+5% green / >0% yellow; NBA PPG ≥118 green / ≥113 yellow; O/U sport-specific thresholds. `ttPace` shows team pace delta from league avg. `k` keys: `ttTeamRPG`, `ttOppERA`, `ttH2HHR`, `ttPark`, `ttOu`, `ttTeamOff`, `ttOppDef`, `ttPace`, `ttOpp` — all in `xcell` handler and `COL_TIPS`.
