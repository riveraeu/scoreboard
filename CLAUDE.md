...
- **WNBA Moneyline + Spread (v1, 2026-05-20)** — `KXWNBAGAME` / `KXWNBASPREAD`. Reuses `simulateNBAJoint` with per-team σ=11 (matches WNBA total sim). Lambdas captured into `_wnbaMlContext` from the WNBA branch of the game-total loop. Same gates as NBA. WNBA-specific dc rules mirror NBA: `wnbaHomeLineupNotPosted`/`wnbaAwayLineupNotPosted` via `sportByteam.wnbaStarters.confirmedTeams`, `wnbaHomeHeavyInjury`/`wnbaAwayHeavyInjury` at usage-out ≥ 0.30. Calibration buckets: `wnba|ml`, `wnba|spread`; tabs: `wnba-ml`, `wnba-spread`.
