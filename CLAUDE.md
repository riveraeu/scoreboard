...
| Soccer (World Cup): 1X2 · total · teamTotal · spread · BTTS | Phase-1 one Elo-derived Dixon–Coles Poisson score matrix per game → all 5 families (`buildScoreMatrix` + projections) — **shadow-only since 2026-06-21** | National-team World Football Elo → goal supremacy (μ=2.7, C=160, ρ=−0.13, neutral); 1X2 incl. draw. Settles on 90'+stoppage (no ET). Phase 2 = attack/defence ratings + host bump + club leagues |
