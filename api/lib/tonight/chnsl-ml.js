// api/lib/tonight/chnsl-ml.js
// emitChnslMlPlays — Chinese Super League game winner (3-way home/away/tie), Phase 1,
// model-free. Fourth model-free maker league, same shape as club-soccer-ml.js (MLS) /
// brasileirao-ml.js / nwsl-ml.js.
//
...
// module only needs a real gameTime (fetched from ESPN via api/lib/chnsl.js), the parsed
...
//
// Output goes into a dedicated `chnslPlays` array (NOT the shared `plays` array) so these rows
// bypass prop/total dedup, the gameTime filter, and the frontend card builder. tonight.js
// merges chnslPlays into the shadow:staging payload only; the client never sees them.
// `chnsl|game` is not in the category gate.

import { CAPTURE_GATE, CAPTURE_CAP } from "../config.js";
import { getChnslSchedule } from "../chnsl.js";

export async function emitChnslMlPlays(ctx) {
  const { chnslMarkets, chnslPlays, cutoffStr, cache, isBustCache } = ctx;
  if (!chnslMarkets || chnslMarkets.length === 0) return;

  // Group every priced side by event so one schedule lookup serves all of that game's sides.
  const events = new Map();
  for (const m of chnslMarkets) {
  ...
  await Promise.all(dateStrs.map(async (dateStr) => {
    schedulesByDate.set(dateStr, await getChnslSchedule({ dateStr, cache, isBustCache }));
  }));
  ...
  for (const [eventTicker, ev] of events) {
    ...
    for (const s of ev.sides) {
      ...
      chnslPlays.push({
        sport: "chnsl", stat: "game", gameType: "game", modelVersion: "chnsl-modelfree-v1",
