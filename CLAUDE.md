...
- `mls.js` — MLS (Major League Soccer) game winner (KXMLSGAME, adopted 2026-07-23, **model-free** — no probability model at all; see `project_maker_modelfree_clubsoccer_2026_07_23` memory). Exists purely to supply a real `gameTime` (fetched from ESPN's `usa.1` scoreboard — Kalshi's own market timestamps aren't reliable kickoff proxies on far-out listings; this is the module that first surfaced the wider gameTime:null bug now fixed across most other Phase-1 modules too, see below) and a 3-way (home/away/tie) resolver (`fetchMlsResults`, ties included — unlike the binary nba-summer/lmb winner grading). `tonight/club-soccer-ml.js` is the emit path: `dcQualified: false` deliberately (unlike every other shadow-only module) so these rows never enter the taker-bettable universe
- `brasileirao.js` — Brasileirão Série A (Brazil) game winner (KXBRASILEIROGAME, adopted 2026-07-23), same model-free playbook as `mls.js` one league later — picked as the highest-liquidity confirmed-real league from the two-track historical sweep (36/36 markets real two-sided books, volume up to ~30k contracts). ESPN `bra.1` scoreboard for `gameTime` + the 3-way resolver (`fetchBrasileiraoResults`). Needs its OWN parse branch/array in `tonight.js` (`brasileiraoMarkets`/`gameType:"brasileiraoMl"`) rather than reusing MLS's `clubSoccerMl` dispatcher, since each row must resolve off a different ESPN league endpoint and carries no per-row sport tag. `tonight/brasileirao-ml.js` is the emit path, same `dcQualified:false` convention as MLS
- `nwsl.js` — NWSL (KXNWSLGAME, adopted 2026-07-23), 3rd model-free maker league, same playbook one league later. **Found that the bare-prefix tickers checked during the 7/23 historical sweep (`KXPREMIERLEAGUE`, `KXUCL`, `KXLALIGA`, etc.) are season-long futures markets, NOT per-game winner markets** — the real per-game equivalent uses a `GAME` suffix (`KXUSLGAME`, `KXNWSLGAME`, `KXCHNSLGAME`, `KXKLEAGUEGAME`). Picked NWSL over `KXUSLGAME` (also 36/36 real books) because USL has a genuine ESPN-side team-abbreviation collision (Louisville City FC and Loudoun United FC both `"LOU"`) — NWSL had zero collisions. ESPN `usa.nwsl` scoreboard; `tonight/nwsl-ml.js` emit path. Registry has 14 of 16 teams — Angel City FC and Racing Louisville hadn't appeared in a live Kalshi ticker yet as of ship date, so their Kalshi abbr wasn't guessed (add when first seen)
...
- `kalshi-settlement.js` — Kalshi-settlement-based shadow_plays grading, **DRY-RUN ONLY** (2026-07-23). `deriveKalshiSide`/`wonFromKalshiResult`/`resolveRowViaKalshi` — a universal alternative to the per-sport ESPN resolvers, graded off each row's own `kalshi_ticker`/`kalshi_side` (new `shadow_plays` columns) instead of team/name matching. Any settlement `result` other than `"yes"`/`"no"` (`"scalar"` — confirmed across MLB props, tennis, golf, LMB/MLS postponements) means void/don't-grade, never a guess. Wired into `/api/shadow-resolver` as a comparison-only pass (see docs/INFRA.md) — not authoritative until agreement is verified on a real sample
...
- `tonight/club-soccer-ml.js` — MLS game winner (KXMLSGAME, adopted 2026-07-23), **model-free** (no probability, see `api/lib/mls.js` above) — the module that first surfaced the gameTime:null bug and was fixed first; now one of 8 (with tennis/soccer/soccer-advance/fight/nba-summer/lmb-ml/nascar) that populate a real `gameTime`
- `tonight/brasileirao-ml.js` — Brasileirão Série A game winner (KXBRASILEIROGAME, adopted 2026-07-23), same model-free playbook as `club-soccer-ml.js` above, one league later — see `api/lib/brasileirao.js`
- `tonight/nwsl-ml.js` — NWSL game winner (KXNWSLGAME, adopted 2026-07-23), 3rd model-free maker league — see `api/lib/nwsl.js`
