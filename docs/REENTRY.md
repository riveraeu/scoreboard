...
**1. A new market class with real books** — not a new stat family in a league already modelled, a class nobody prices well. Required before any modelling: real two-sided books (bid-ask within `CAPTURE_MAX_SPREAD`, overround `avgMarket × sides` ≈ 1 — thin ≠ exploitable; the dead-book artifacts `totalBases` 94¢ and segment overround manufactured up to +0.39 of fake Brier skill); a resolvable settlement source (Kalshi settlement grading removed the ESPN dependency, unblocking the KBO/NPB/CFL class); **a per-game market shape** (one row = one game's outcome — this repo's whole capture/analysis stack, `shadowId`, day-clustered CIs, the pre-game `gameTime` gate, assumes it; a season-long/futures-shaped contract with one outcome for the whole season has no per-game structure to hang any of it off, same exclusion class as every other season-futures dismissal); and enough volume to matter at the size actually traded. **`KXNFLTSPEC` was the candidate named here — DISMISSED 2026-09-01**: live-checked with 306 real-book markets, 8¢ median spread, ~$61.8k volume (liquidity was never the issue), but every market shares one of two end-of-season `closeTime`s with no per-game ticker segment — it's a season-long achievement bundle ("did X ever happen in one game this season"), not the per-game class this condition requires. No other NFL prop catalyst is currently identified; the September-NFL opening this section described is not yet satisfied by anything on the board.
