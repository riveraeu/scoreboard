...
**SeasonRate blend (added 2026-05-26)**: post-sim, `truePct` is blended toward the empirical threshold hit rate from the player's own gamelog. `truePct = (1-w) × simPct + w × refRate` where `refRate = (seasonPct + softPct) / 2` (soft-tier rate folded in when present, matching the no-sim fallback formula) and `w = min(1, sample/20) × 0.5` (max 50% weight at 20+ games). Counters pure-sim drift from empirical hit rate at the threshold — particularly visible when a player's recent matchup adjustments (USG, DvP, pace) inflate simPct above what season + soft-tier rates support. Applied to NBA / WNBA / NHL player props and MLB strikeouts (where `refRate` uses `primaryPct + softPct` instead of `seasonPct`). MLB HRR skipped — the logit-sigmoid model is already rate-based at its core (base = `(primaryPct + softPct) / 2` with park/OPS/WHIP/barrel multiplicative shifts).
