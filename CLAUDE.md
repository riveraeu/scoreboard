...
**Model version toggle (`v1` / `v2`)**: Per-user localStorage flag (`scoreboard_modelVersion`) selects which model the frontend asks for. `useModelVersion()` in `src/lib/hooks.js` is the canonical accessor. App.jsx appends `&model=v2` to all `/api/tonight` URLs when v2 is active, and `trackPlay` stamps every pick with the active `modelVersion`. Server-side `/api/tonight` reads `params.get("model")`, coerces to `v1 | v2`, and echoes back in the response. **Server returns identical play data for both versions** — no server-side branching on `modelVersion`. **Client-side filters diverge**:
- **v2**: `dcQualified === true && edge >= 5` (data-confidence-based qualification)
- **v1**: `qualified !== false && finalSimScore >= 8 && hitterFinalSimScore >= 8` (SimScore-based)
- Client `EDGE_GATE = 5` (App.jsx:27, LineupsPage.jsx:103); server `EDGE_GATE = 3` for calibration continuity — every pick is stored, then v2 client further filters to edge≥5.
- Different explanation block rendered in the play card per version.
- The originally-planned "SimScore-as-attribution refactor" did not happen — once dataConfidence existed as a parallel score, v2 became "trust dc + edge ≥ 5" instead of "rewrite SimScore." Same server machinery, different client gate.

`/api/auth/calibration` accepts `?modelVersion=v1|v2|all` (default `all`) and always returns a top-level `byModelVersion: { v1, v2 }` breakdown — picks tracked before the toggle have no field and are treated as v1. **v1 deprecation deferred to ~2026-05-27** (Phase 1 week-2 review) — defer the ~200-LOC client+server cleanup until v2 calibration data validates against v1.
