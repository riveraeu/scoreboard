...
- `/api/kalshi-order` — `POST`, authenticated. Body: `{ ticker, side: "yes"|"no", price: int (1–99), count: int }`. Signs via **RSA-PSS / SHA-256 / saltLength=32** with `KALSHI_API_KEY_ID` + `KALSHI_PRIVATE_KEY`. **Signature message: `timestamp_ms + "POST" + "/trade-api/v2/portfolio/orders"`** (ms timestamp, path only, no body). Supports PKCS#1 and PKCS#8 PEM. Returns the full Kalshi `order` object; the frontend reads `taker_fill_count`/`taker_fill_cost` and stamps the tracked pick with the real fill (cost→`units`, avg price→`americanOdds`, plus `kalshiCount`/`kalshiAvgCents`/`kalshiRestingCount`). Filled cost/odds verified to match Kalshi to the penny 27/27 picks (2026-05-30 via `/api/kalshi-fills`); resting (0-fill) orders track at intended size with a note. **"Place All" batch (2026-05-31):** a ⚡ button in the `LineupsPage` toolbar (leftmost action button, before Picks; muted Research-style styling — `#30363d` border / transparent bg / `#484f58` text; shown only when `authEmail` + ≥1 candidate) opens a confirm modal and, on confirm, POSTs `/api/kalshi-order` sequentially (await each — avoids 429s) for every candidate, tracking each pick with its real fill via the same override shape as the single flow. Candidate set = `tonightPlays` (qualified) minus already-tracked (`trackIdFor` exported from `LineupsPage.jsx`) minus started games (`gameTime ≤ now`) minus plays where `_placeAllSizing` returns null (no `kalshiTicker` or ⅛-Kelly rounds to 0 contracts — e.g. heavy favorites). Sizing (`_placeAllSizing` in App.jsx) mirrors the track-confirm modal's ⅛-Kelly on the **side actually bet** (UNDER uses `noTruePct` + `noKalshiPct`), $500 cap / $30 fallback. Modal warns on cash shortfall (`kalshiBalance`) but still allows placement — Kalshi rejects underfunded orders individually (row marked ✗).
  - **Pre-flight validation (Flow A, 2026-06-01):** every candidate runs through `validateCandidate(play, sizing)` in `src/lib/placeValidation.js` → `{ hard, soft }`. **HARD** failures exclude the pick from the batch (shown ✗ greyed, never ordered); **SOFT** failures annotate it (⚠) but it still places. `placeAllCandidates` keeps all rows (for display) + attaches `validation`; `placeAllPlaceable` = rows where `hard.length === 0`. **`placeAllPlaceable` is the single source of truth for all four count surfaces**: toolbar badge, tab badges, card badges, and the modal — so they always agree. **Hard checks:** re-assert `dcQualified && edge>=EDGE_GATE_CLIENT` (catches slate drift since modal open), `kalshiTicker` present, `count>=1`, price in range, distinct matchup teams. **Soft checks:** slippage ≥ 3¢ over top-of-book (from `rawKalshiPct`/`rawNoKalshiPct`) or thin/wide-market proxy. Calibration gates removed (2026-06-04) — all active-gate categories are ROI-positive per shadow calibration. **`placeAllPlaceableIds`** (`Set<trackId>`) + **`placeAllCountByDay`** (per-PT-date count) are derived from `placeAllPlaceable` and passed to `LineupsPage`. Tab badges show `placeAllCountByDay[date]` when logged in; card display via `playsForGame(allTonightPlays, game, trackedIds, activePicks, placeAllPlaceableIds)` — the 5th param filters out untracked plays not in the set, so card counts match tab badges exactly. **Started-game filter (2026-06-04):** tab badge `qualifiedByDay` and `MatchupCard.untrackedBadge` both skip plays whose `gameTime ≤ Date.now()` (or `gameState === 'in'|'post'`). **`activeDayTab`** (the selected date pill on LineupsPage) is lifted to `App.jsx` state so the modal and page share the same selected day. New `/api/tonight` emits for slippage: `rawKalshiPct`/`rawNoKalshiPct` (pre-orderbook-blend top-of-book, stashed before the blend overwrite in `tonight.js`) and `homeAwayResolved` (`authoritative`|`fallback`|null). Flow B (LLM auditor on passed-but-flagged picks) is the planned next layer. **Today-only filter (2026-06-04):** modal opens showing all placeable plays across all days; a "Today only" checkbox (default **unchecked**, resets on each modal open) filters the list + cost + order scope to plays whose PT game date matches literal today — not the selected tab date. Checkbox is hidden once placement completes. `runPlaceAll` takes `cands` as a parameter; toolbar badge counts the full unfiltered `placeAllPlaceable`.
...
**Spread alt-line dedup + category gate interaction (updated 2026-06-05)**: The dedup key is `sp|sport|seg|sortedTeams|line|gameDate`. Different alt lines (e.g. +2.5 vs +3.5) compete independently. Both sides of the same line (MIN +3.5 vs KC -3.5) share one key; the higher-edge side wins. The remaining edge case: the dedup winner (e.g. PHI -2.5 at 19% edge, truePct 38%) has higher raw edge but fails the category gate (truePct < 80%), while the demoted loser (CWS +2.5 at 16% edge, truePct 83%) qualifies. Fix (2026-06-05): `passesGate` (LineupsPage.jsx) and `_qualifiedFilter` (App.jsx) changed from blanket `_altLineDemoted → false` to `_altLineDemoted && !passesCategoryGate(p) → false` — demoted plays that pass the category gate are allowed through because opposite-side truePcts are complementary (~sum to 100%), so you can never show both sides of the same line simultaneously (one side ≥80% forces the other ≤20%). Market Report symptom: bold play visible but absent from LineupsPage card.
