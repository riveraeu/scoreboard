                  ...
                  <div key={playKey}
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:8,marginTop:4}}>
                      ...
                      {play.sport === "mlb" && (() => {
                        ...
                        const _cRPGPts = combinedRPG == null ? 1 : combinedRPG >= 10.5 ? 2 : combinedRPG >= 8.5 ? 1 : 0;
                        ...
                        return (
                          <div style={{background:"#0d1117",borderRadius:8,padding:"8px 10px",fontSize:11,color:"#8b949e",lineHeight:1.65}}>
                            ...
                            {combinedRPG != null && <>{" "}<span style={{color:"#8b949e"}}>Combined road scoring:</span>{" "}<span style={{color:isUnder?(combinedRPG<=8.5?"#3fb950":combinedRPG<=10.0?"#e3b341":"#f78166"):(combinedRPG>=10.5?"#3fb950":combinedRPG>=8.5?"#e3b341":"#f78166"),fontWeight:600}}>{combinedRPG.toFixed(1)} runs/game</span><span style={{color:"#8b949e"}}>{isUnder?(combinedRPG<=8.5?" — low combined offense":combinedRPG<=10.0?" — moderate scoring":" — high combined offense"):(combinedRPG>=10.5?" — high combined offense":combinedRPG>=8.5?" — above-average scoring":" — below-average offense")}.</span></>}
              ...
              return (
                <div key={playKey}
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:8,marginTop:play.sport==="mlb"?0:8}}>
                        ...
                        {play.sport === "wnba" && (() => {
                          const statName = { points:"points", rebounds:"rebounds", assists:"assists", threePointers:"3-pointers" }[play.stat] || play.stat;
                          const displayRank = play.posDvpRank ?? play.oppRank;
                          const displayValue = play.posDvpValue ?? play.oppMetricValue;
                          const sc = play.wnbaSimScore;
                          const first = play.playerName.split(" ")[0];
                          const scColor = sc != null ? (sc >= 8 ? "#3fb950" : sc >= 5 ? "#e3b341" : "#8b949e") : "#8b949e";
                          const rankColor = play.isHardMatchup ? "#f78166" : (displayRank != null && displayRank <= 10) ? "#3fb950" : (displayRank != null && displayRank <= 15) ? "#e3b341" : play.softPct !== null ? "#3fb950" : "#c9d1d9";
                          ...
                          const paceColor = play.wnbaPaceAdj == null ? "#8b949e" : play.wnbaPaceAdj > 0 ? "#3fb950" : play.wnbaPaceAdj > -2 ? "#e3b341" : "#f78166";
                          ...
                          const paceDesc = play.wnbaPaceAdj == null ? null : play.wnbaPaceAdj > 2 ? "a fast game — more possessions, more opportunities to score" : play.wnbaPaceAdj > 0 ? "slightly above-average pace" : play.wnbaPaceAdj > -2 ? "slightly slower pace" : "a slow game — fewer scoring opportunities";
                          const rankDesc = displayRank == null ? null : displayRank <= 3 ? "one of the worst defenses in the league" : displayRank <= 8 ? "a weak defense" : displayRank <= 15 ? "a soft matchup" : null;
                          const _usgPts = play.stat === "rebounds"
                            ? (play.wnbaOpportunity == null ? 1 : play.wnbaOpportunity >= 27 ? 2 : play.wnbaOpportunity >= 22 ? 1 : 0)
                            : (play.wnbaUsage == null ? 1 : play.wnbaUsage >= 27 ? 2 : play.wnbaUsage >= 22 ? 1 : 0);
                          const _c1Label = play.stat === "rebounds"
                            ? `AvgMin: ${play.wnbaOpportunity != null ? play.wnbaOpportunity.toFixed(0)+"m → "+_usgPts : "—"}/2`
                            : `USG%: ${play.wnbaUsage != null ? play.wnbaUsage.toFixed(1)+"% → "+_usgPts : "—"}/2`;
                          const _dvpPtsPC = play.dvpRatio != null ? (play.dvpRatio >= 1.05 ? 2 : play.dvpRatio >= 1.02 ? 1 : 0) : 1;
                          const _wnbaSeasonHRPtsPC = play.wnbaSeasonHitRatePts ?? (play.seasonPct >= 90 ? 2 : play.seasonPct >= 80 ? 1 : 0);
                          const _wnbaSoftHRPtsPC = play.wnbaSoftHitRatePts ?? (play.softPct == null ? 1 : play.softPct >= 90 ? 2 : play.softPct >= 80 ? 1 : 0);
                          const _ouPtsPC = play.wnbaTotalPts ?? 1;
                          const scTitle = sc != null ? [_c1Label,`DVP: ${_dvpPtsPC}/2`,`Season HR: ${_wnbaSeasonHRPtsPC}/2`,`Tier HR: ${_wnbaSoftHRPtsPC}/2`,`Game Total: ${_ouPtsPC}/2`].join("\n") : null;
                          return (
                            <div style={{background:"#0d1117",borderRadius:8,padding:"8px 10px",fontSize:11,color:"#8b949e",lineHeight:1.65}}>
                              <div>
                                {play.wnbaOpportunity != null ? <>{first} averages <span style={{color:minColor,fontWeight:600}}>{play.wnbaOpportunity.toFixed(0)} minutes</span> a night{minDesc ? <span style={{color:"#8b949e"}}> — {minDesc}</span> : ""}{play.stat === "assists" && play.wnbaAvgAst != null ? <> (<span style={{color:play.wnbaAvgAst>=7?"#3fb950":play.wnbaAvgAst>=5?"#e3b341":"#f78166",fontWeight:600}}>{play.wnbaAvgAst.toFixed(1)} APG</span>)</> : play.stat === "rebounds" && play.wnbaAvgReb != null ? <> (<span style={{color:play.wnbaAvgReb>=9?"#3fb950":play.wnbaAvgReb>=7?"#e3b341":"#f78166",fontWeight:600}}>{play.wnbaAvgReb.toFixed(1)} RPG</span>)</> : play.wnbaUsage != null ? <> (<span style={{color:play.wnbaUsage>=27?"#3fb950":play.wnbaUsage>=22?"#e3b341":"#f78166",fontWeight:600}}>{play.wnbaUsage.toFixed(0)}% USG</span>)</> : ""}.</> : null}
                                {" "}{first} hits this line in <span style={{color:seasonColor,fontWeight:600}}>{play.seasonPct}%</span> of games{play.seasonGames ? <span style={{color:"#484f58",fontSize:10}}> ({play.seasonGames}g)</span> : ""}.
                                ...
                                {play.wnbaPaceAdj != null && <>{" "}Game pace is <span style={{color:paceColor,fontWeight:600}}>{play.wnbaPaceAdj > 0 ? "+" : ""}{play.wnbaPaceAdj}</span> possessions above average — {paceDesc}.</>}
                                {play.wnbaGameTotal != null && <>{" "}Game total <span style={{color:play.wnbaTotalPts>=2?"#3fb950":play.wnbaTotalPts>=1?"#e3b341":"#8b949e",fontWeight:600}}>{play.wnbaGameTotal}</span><span style={{color:"#8b949e"}}>{play.wnbaGameTotal>=175?" — a high-scoring slate":play.wnbaGameTotal>=168?" — above-average scoring":play.wnbaGameTotal>=158?" — an average total":" — a low-scoring slate"}.</span></>}
                                {play.wnbaBlowoutAdj != null && play.wnbaBlowoutAdj < 0.99 && <>{" "}<span style={{color:"#f78166",fontWeight:600}}>Blowout risk</span> — large spread reduces model mean by {Math.round((1-play.wnbaBlowoutAdj)*100)}%.</>}
                                {play.isB2B != null && <>{" "}{play.isB2B ? <><span style={{color:"#f78166",fontWeight:600}}>Back-to-back</span> — model applies a scoring reduction.</> : <>Fully rested tonight.</>}</>}
                                {sc != null && <>{" "}<SimBadge sc={sc} scTitle={scTitle} scColor={scColor} /></>}
                              </div>
                            </div>
                        ...
                        {play.oppRank && play.stat !== "strikeouts" && play.sport !== "nba" && play.sport !== "wnba" && play.sport !== "nhl" && play.sport !== "mlb" && (
