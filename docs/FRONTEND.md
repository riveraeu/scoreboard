...
- **DoThisBanner** (+ its full `_doThisCandidates` dependency chain, `MODEL_NEXT` roadmap data, etc.) — ported verbatim into this file 2026-07-22 when `/model`/`ReportPage.jsx` was deprecated and deleted (see `project_do_this_banner` memory). The fall-through priority ladder, tiers 1 → 5: (1) actionable data health, (1.6) maker adverse-selection red flag, (**1.8 REMOVED 2026-07-28** — was the shadow maker V2 lifecycle; see below), (2) taker betting-board changes, (2.2) accuracy changes, (2.5) validate ripe shadow models, (3/3.2) build-next roadmap (currently inert — every `MODEL_NEXT` entry is shipped or infra-blocked, self-advances when a new one is added), (3.15) derive a per-category bet window, (3.25) vet shortlisted markets, (3.5) triage detected markets, (3.6) sportsbook regime-change tripwire, (4) `SCHEDULED_CHECKPOINTS` dated follow-ups, (5) quiet-day floor. Reads the full `shadowReportData` payload (`accuracyBoard`/`bettingBoard`/`makerBoard`/`newMarkets`/`shortlistedMarkets`/`sportsbookValidation`/`dataHealth`) — those boards are no longer displayed anywhere, but the server still produces them and the banner still reads them.
...
**Layout pass 2026-07-28** — three fixes, all presentation-only (no measurement/logic touched): (1) `EquityCurve` + `BandLadder` were each `maxWidth:560` stacked inside a `maxWidth:1280` container, so the whole right half of the page below the tile rows was empty while both figures rendered at under half their available width; they now share a `repeat(auto-fit, minmax(380px, 1fr))` grid (re-stacks to one column under ~800px, so narrow/mobile is unchanged) and the 560 caps are gone. (2) `DoThisBanner` clamps `why` to two lines behind a "Why ▼" toggle — the terminal tiers run ~600 chars, which made the longest paragraph on the page outrank its own headline — and a `dim`/`gray` tone now renders the headline in `C.text`: what makes a no-action tier quiet is the absence of an accent frame and of anything to click, not low contrast on the words. (3) `MakerUtilization` collapses to its summary line while **disarmed** (takes `armed` from `boardData`), since with no resting orders every per-sport row is an empty track repeating `0/N resting`; rows return expanded-by-default once armed, when eligible-vs-resting is a real cap-pressure diagnostic.

**`STRATEGY_CLOSED` (2026-07-28)** — a const in `MakerBoardPage.jsx` suppressing the five ladder tiers that nag model tuning: **1.6** (maker adverse selection — remedy is a band slice), **2** (promote/pull — gate empty, PROMOTE is an in-sample screen), **2.2** (Improve inputs / Recalibrate / Diagnose — all residual search), **2.5** (tune:gate validation), **3.15** (derive a bet window — a price-band slice). All five were authored while taker-side model improvement was the daily program; `docs/REENTRY.md` now lists their actions as **disqualifying**, not merely unproductive — "a per-category or per-sport breakout … slicing is 0-for-6" and §2's "a model change with a stated mechanism … **not** a correction discovered by searching residuals", which is exactly what `tune:residual` does. Surviving tiers: 1 (data health), 3.25/3.5 (vet + triage new markets — condition #1, the only thing that could justify re-entry), 3.6 (regime tripwire), 4 (checkpoints), 5 (floor, copy rewritten off the retired arm-criterion framing). Kept as a const rather than deleted code to match `SHELVED`/`TAPE_REPLAY_ENABLED` and the doc's own posture — instruments stay, actions stop, re-entry is a one-line revert with the conditions in front of whoever does it. **Verified against the live report**: primary becomes `Vet 16 shortlisted markets`, with triage + 8 overdue checkpoints on the `Then ·` line.

**The banner had gone blind (2026-07-28)** — when V2 was shelved, tier 1.8 was rewritten from an arming ladder into a terminal notice but kept its `if (mb) out.push(...)` form. `makerBoard` is always in the payload, so the notice became the **primary candidate every day, permanently**, and since the banner renders only `primary`, tiers 2 → 5 were structurally unreachable. Measured against the live report the day it was found, it was burying: 1 accuracy action (`mlb|totalBases` Diagnose), 16 shortlisted markets, 15 detected markets, and **8 overdue checkpoints** (oldest `2026-07-23`). Three fixes: (a) tier 1.8 deleted — **a status must never hold an action slot; any future tier here needs a real firing condition** — with the shelved fact moved next to the arm state in the Open-positions section head, rendered off `board.shelved` (the server's own `MAKER_V2_SHELVED`, exported from `api/lib/maker-live.js` so the one-line un-shelve revert clears the UI too); (b) the queued `rest` renders again as a `Then · …` line — it had been copy-text-only, visible only to someone who clicked the copy icon and pasted it somewhere, and it is deliberately **not truncated**; (c) the `next clock` tile now shows **overdue-first** (`checkpoints due · N`, amber, oldest date) — it filtered `c.date > todayPT` only, so it structurally skipped everything already due and displayed "Jul 29" while 8 were overdue. Both of the page's independent checkpoint surfaces were simultaneously reporting that nothing was due, which is precisely the failure a dated-follow-up mechanism exists to prevent.

**EquityCurve x-axis (2026-07-28)** — the curve had no date axis at all (nine points, no way to tell which day was which without hovering). The svg now carries a `AXIS`-height band *below* the plot area (`H = PH + AXIS`, `ys()` maps into `PH` only, so the series wasn't compressed to make room) holding `M/D` labels from `makerBoard.daily[].day`. Labels are thinned by `step` against a `LABEL_W` footprint so they can never collide as the series lengthens, the last day is always kept, and end labels anchor inward so they can't clip. `fmtAxisDay` renders an unrecognized `day` raw rather than guessing — a mis-parsed date label is worse than an ugly one. No axis rule and no tick marks: the dots already mark the x positions, and a second horizontal rule under the zero line reads as a second baseline.

**Real-vs-paper dollar disambiguation (2026-07-28)** — the page shows two cumulative dollar totals ~250px apart that differ by ~230× (`+$6.65` real V2 vs `+$1534.51` V1 paper), previously distinguished only by 9px sub-labels. The gap is **population, not performance**: V1 simulates quoting every eligible ticker on every cron tick, V2 only ever rested orders on a capped subset. Three changes, on the principle that a disclaimer has to live on the number a reader actually scans: the `EquityCurve` endpoint label carries the word "paper" inline and drops to `C.gray` (real money is the brighter figure); the `PortfolioTiles` V2 tile is labeled `V2 PnL · real money` with `real capital ·` leading its sub-line; and `MakerProgress` states the relationship under its section head with both graded-fill counts. Don't "simplify" either total to a bare number.
