...
- `src/components/MatchupCard.jsx` — per-game card: team logos, center stats, NBA lineup in collapsible drawer (lazy-fetched on first open); umpire and weather data are still fetched and used in True% but no longer displayed in the card UI (removed Apr 2026); **NBA injury badges removed from card** — moved to team/player page (Apr 2026); home team lineup right-aligned inside drawer. **MLB cards show header only** — pitcher row, confirmed/expected lineup badge, and batting lineup drawer all removed (Apr 2026). **Center header** is state-aware: `pre` → game time + O/U + ML + spread; `in` → score (`awayScore – homeScore`) + period/inning/status in yellow; `post` → score + "Final" in gray. Score data comes from `game.gameState/gameDetail/homeScore/awayScore` (seeded by sport-specific `gameScores` via `buildGames` — works for MLB, NBA, NHL). **Play badge + inline drawer**: when `gamePlays.length > 0`, shows a badge below the header — blue `▶ N plays` (untracked), gold `★ N plays` (all tracked); clicking blue toggles an inline `PlaysColumn` under the card; clicking gold opens the global picks drawer. `allTracked = trackedCount === totalPlays`. Props: `{ game, mlbMeta, mlbMetaTomorrow, nbaMeta, navigateToPlayer, navigateToTeam, gamePlays, allTonightPlays, trackedPlays, trackPlay, untrackPlay, navigateToPlay, navigateToModel, expandedPlays, setExpandedPlays, openPicksDrawer }`. **Date-aware meta selection**: `isToday` computed from `gameDate` vs PT date — used to select MLB game odds (`mlbGameOdds = isToday ? mlbMeta?.gameOdds : {}`).
...
- `nhlMeta` — object in `/api/tonight` response: `{ gameScores: {abbr: {homeTeam, awayTeam, state, detail, homeScore, awayScore, gameDate, gameTime}} }`. Built from NHL ESPN scoreboard via `parseGameScores`. Used by `buildGames` to seed NHL game cards with live/final scores and correct home/away assignment.
