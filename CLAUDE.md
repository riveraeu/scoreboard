...
- `shadow.js` — `/api/shadow-snapshot`, `/api/shadow-resolver`, `/api/shadow-pregame-snap`, `/api/shadow-report`, `/api/{polymarket,sportsbook}-deltas`, `/api/routine-note`
...
- `sportsbook.js` — sharp-book reference feed (The Odds API / Pinnacle, shadow-only). `fetchSportsbookGames` (no key → clean `[]` no-op), `devigTwoWay`, `normalizeOddsEvent`
...
- `tonight/sportsbook-deltas.js` — `emitSportsbookDeltas`: de-vigged sharp-book ML vs Kalshi (`deltaCents = bookFairPct − kalshiPct`, + = Kalshi cheap = lagging). Liquidity-gated (drops untraded sides). Shadow + `?debug=1` only
...
| `/api/polymarket-deltas` | `shadow.js` | GET (ADMIN/JWT) — multi-day Kalshi-vs-Polymarket ML divergence. `?days=` `?sport=`. `exec` = book-walked surviving edge after slippage |
| `/api/sportsbook-deltas` | `shadow.js` | GET (ADMIN/JWT) — multi-day Kalshi-vs-sharp-book ML divergence. `meanSigned>0` = Kalshi systematically cheap = lag edge. `?minAbs=N` adds a `rows` tail dump (\|Δ\|≥N¢, LIMIT 200) for outlier audits. Populated only once `THE_ODDS_API_KEY` is provisioned |
