...
| `/api/auth/shadow-analysis` | `auth.js` | Analyses (thresholdRankRoi, clvAnalysis, capRoi, edgeBucketRoi, …). `?residual=<sport\|stat>` dumps raw rows for `npm run tune:residual`. `?clvCaptureHistory=1` / `?clvDay=` for CLV forensics. `extractFeatures` folds a uniform `x*` exogenous namespace + `snapshot_date` onto every play (feeds `tune:residual` / `tune:learncurve`) |
