...
**Fetch fan-outs must be concurrency-capped**: unbounded `Promise.all` fetch loops exhaust the instance's file descriptors on big slates (`TypeError: fetch failed`, cause `EMFILE`/`EBUSY`; once exhausted ALL outbound fetches in the run fail — ESPN and Upstash alike, while single-fetch routes and the snapshot cron stay healthy). Wide fetch loops go through `pLimit(n)` (`api/lib/utils.js`, n≈16–20); per-key KV reads over lists use `cache.getMany` (chunked MGET in `makeCache`; feature-test `CACHE2.getMany` + parallel-GET fallback). Details → `docs/INFRA.md` § Data Plumbing Gotchas.
