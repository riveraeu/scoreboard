...
0. **StatusStrip + Data health** (`StatusStrip`/`DataHealth`) — pipeline trust header; unchanged.
1. **DO THIS** (`DoThisBanner`) — the fall-through priority ladder in `_doThisCandidates(d)`. Two maker tiers added 2026-07-19: **1.6 adverse-selection red flag** (graded fills n≥30 AND filled-vs-quoted side-won delta ≥8pp, or fill-PnL CI entirely negative — the one failure mode that kills the maker strategy claims the morning headline) and **2.05 ARM DECISION** (graded ≥ `armCriterion.minFills` 200 AND `pnlLoCI`>0 — review V2 scope; human decision, never auto-armed). The legacy tiers (betting changes 2, accuracy 2.2, validate-ripe 2.5, window 3.15, roadmap 3/3.2/3.25/3.5, checkpoints 4, all-clear floor 5) still read the `accuracyBoard`/`bettingBoard` PAYLOAD (which the server still produces); the tier-5 floor now leads with the maker accrual count.
2. **MAKER PROGRESS** (`MakerProgress`, 2026-07-19) — the strategy readout, replacing the legacy board table. Pieces: **ArmTile** (hero: fill PnL ¢/contract + CI-lo, progress meter to the 200-graded-fill arm bar, status-colored accruing/on-track/straddling/negative; border + label flip green when the arm criterion is met) · **fill-rate / adverse-selection / next-clock tiles** (`Tile`) · **EquityCurve** (cumulative graded paper PnL by day — single-hue SVG line, zero baseline, per-point `<title>` tooltips, one direct label at the last point; placeholder until fills span 2 days) · **BandLadder** (per-ask-band 80-84/85-89/90-96: bar length = fills, signed colored ¢/ct margin label — the V2 targeting view). Data: `d.makerBoard` {quotes, fills, quotedOutcomes, daily, bands, armCriterion}. Color doctrine: no categorical series palette — single hue for magnitude marks, green/red only on signed values (+/− prefix is the non-color channel; the red/green pair alone fails CVD ΔE).

**Legacy boards REMOVED from display 2026-07-19** (strategy pivot: no taker edge exists venue-wide, so "which model beats the market?" is no longer the page's question): `ModelSummary` (the fused accuracy/betting table), `AccuracyBoard`, `BettingBoard`, `ModelNext`, `GateDigest`, `CrossVenueValidation`, `PriceBands`, `CalibBandsTable` + their cell/badge helpers — most were already dead code after earlier strips; all deleted. The SERVER payload is untouched: accuracyBoard/bettingBoard/brief still generate (the banner tiers, daily brief consumers, and tune CLIs read them), so restoring a display is a frontend-only change. Category-level model drill-downs now live in the CLIs (`tune:gate`, `tune:residual`, `?brier=1`).
