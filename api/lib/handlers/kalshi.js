...
export async function handleKalshiRoutes(ctx) {
  ...
  if (path === "maker-v2-arm" && method === "POST") {
    ...
    // Checked explicitly (not just relying on isArmed()'s own SHELVED gate) so a shelved engine
    // returns an explanation rather than a silently-ineffective 200 — setting the KV flag while
    // shelved would look like it worked. Un-shelving is a code change (SHELVED in
    // maker-live.js), by design, not something this endpoint can do on its own.
    if (MAKER_V2_SHELVED) {
      return errorResponse(
        "Maker V2 is SHELVED: no demonstrated fillable edge across six measurements on the "
        + "original 80-84 favorite-band hypothesis; the pre-registered lead-time test was "
        + "rejected on all three criteria. See docs/MAKER_LEADTIME_PREREG.md and docs/INFRA.md. "
        + "To un-shelve, flip SHELVED in api/lib/maker-live.js — deliberately a code change, "
        + "not an API call.", 409);
    }
    await setArmed(CACHE2, true);
    return jsonResponse({ ok: true, armed: true, shelved: false });
  }
