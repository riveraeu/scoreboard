...
**Category gate (2026-06-01)**: UI display + the `push/notify` cron are restricted to categories with confirmed positive shadow ROI. Gate lives in `passesCategoryGate()` in `api/lib/category-gate.js` (re-exported by `src/lib/constants.js`), applied in `App._qualifiedFilter` / `LineupsPage.passesGate` and the cron. Tracked picks bypass it. **Add a category only when `npm run tune:gate` shows ROI>0 at cumulative n≥50 (`MIN_N_PROMOTE`) AND per-band detail is coherent (no in-window band flipping negative)** — band coherence, not raw n, is the real guard. Current allowed gates: **only mlb|strikeouts** (itself frozen→trending-demote). **All three wnba gates PAUSED 2026-06-19** after formula-clean validation: points [70,80) mis-placed (n=3/−36%, profit at [80,85)/+14% excluded above the cap); rebounds [70,85) mis-placed/weak (in-gate n=1, real plays ~breakeven, Brier −0.028 @ n=96); spread overconfident/losing (bet set n=15 hit 20%/ROI −15.5% vs +13.8% claimed edge, unfiltered Brier skill −0.032 @ n=237). The gate collapsed under formula-clean scrutiny (consistent with the session-wide Brier finding that nothing beats the market) — re-enable any only after tune:gate confirms a coherent +ROI band at n≥50; see memory [[project-live-gate-validation-2026-06-19]]. Plus shadow-only categories (mlb|hits, mlb|totalBases, tennis|match) and their per-band history → `docs/MODEL.md` § Category gate.
