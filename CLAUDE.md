...
- `mls.js` — MLS (Major League Soccer) game winner (KXMLSGAME, adopted 2026-07-23, **model-free** — no probability model at all; see `project_maker_modelfree_clubsoccer_2026_07_23` memory). Exists purely to supply a real `gameTime` (fetched from ESPN's `usa.1` scoreboard — Kalshi's own market timestamps aren't reliable kickoff proxies on far-out listings, and every OTHER Phase-1 shadow-only module hardcodes `gameTime:null`, silently blocking `computeMakerQuote`'s pre-game gate; found 2026-07-23, fixed for MLS only) and a 3-way (home/away/tie) resolver (`fetchMlsResults`, ties included — unlike the binary nba-summer/lmb winner grading). `tonight/club-soccer-ml.js` is the emit path: `dcQualified: false` deliberately (unlike every other shadow-only module) so these rows never enter the taker-bettable universe
...
- `tonight/{tennis-match,soccer,soccer-advance,golf-h2h,mlb-outs,nascar,fight,nba-summer,lmb-ml}.js` — the Phase-1 emit modules. Each groups its Kalshi series, emits the favorite side (Kalshi YES in window), and pushes into a **dedicated array** (e.g. `tennisPlays`, `soccerPlays`) merged into `shadow:staging` ONLY — never the client response. They bypass prop dedup / gameTime filter / card builder. **All 8 hardcode `gameTime: null`, which silently blocks `computeMakerQuote`'s pre-game gate (found 2026-07-23) — none of them have ever actually been maker-quotable. Not yet fixed** — tracked follow-up
- `tonight/club-soccer-ml.js` — MLS game winner (KXMLSGAME, adopted 2026-07-23), **model-free** (no probability, see `api/lib/mls.js` above) — the one Phase-1 emit module that DOES populate a real `gameTime` (fetched from ESPN), so it doesn't share the bug above
