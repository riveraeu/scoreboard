// api/lib/tonight/argprem-ml.js
// emitArgPremMlPlays — Argentina Liga Profesional de Fútbol game winner (3-way home/away/tie),
// model-free, adopted 2026-07-24 (found via the 2141-row kalshi_series_seen baseline backlog
// sweep, project_baseline_backlog_2026_07_24 memory). Same shape as club-soccer-ml.js (MLS) /
// brasileirao-ml.js / nwsl-ml.js / chnsl-ml.js / ligamx-ml.js.
//
...
// module only needs a real gameTime (fetched from ESPN via api/lib/argprem.js), the parsed
...
//
// Output goes into a dedicated `argPremPlays` array (NOT the shared `plays` array) so these
// rows bypass prop/total dedup, the gameTime filter, and the frontend card builder. tonight.js
// merges argPremPlays into the shadow:staging payload only; the client never sees them.
// `argprem|game` is not in the category gate.

import { CAPTURE_GATE, CAPTURE_CAP } from "../config.js";
import { getArgPremSchedule } from "../argprem.js";

export async function emitArgPremMlPlays(ctx) {
  const { argPremMarkets, argPremPlays, cutoffStr, cache, isBustCache } = ctx;
  if (!argPremMarkets || argPremMarkets.length === 0) return;

  const events = new Map();
  for (const m of argPremMarkets) {
  ...
  await Promise.all(dateStrs.map(async (dateStr) => {
    schedulesByDate.set(dateStr, await getArgPremSchedule({ dateStr, cache, isBustCache }));
  }));
  ...
  for (const [eventTicker, ev] of events) {
    ...
    for (const s of ev.sides) {
      ...
      argPremPlays.push({
        sport: "argprem", stat: "game", gameType: "game", modelVersion: "argprem-modelfree-v1",
