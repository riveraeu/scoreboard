                      ...
                      {play.sport === "mlb" && (() => {
                        ...
                        return (
                          <div style={{background:"#0d1117",borderRadius:8,padding:"8px 10px",fontSize:11,color:"#8b949e",lineHeight:1.65}}>
                            ...
                            {" "}<span style={{color:"#c9d1d9"}}>{play.homeTeam}</span>'s starter posts{hWHIP != null ? <> a <span style={{color:whipColor(hWHIP),fontWeight:600}}>{hWHIP.toFixed(2)} WHIP</span></> : " — WHIP"} against a <span style={{color:"#c9d1d9"}}>{play.awayTeam}</span> offense at{aRPG != null ? <> <span style={{color:rpgColor(aRPG),fontWeight:600}}>{aRPG.toFixed(1)}</span> RPG</> : " — RPG"}.
                            {h2hTR != null && <>{" "}Combined totals have hit{" "}<span style={{color:h2hTR >= 80 ? "#3fb950" : h2hTR >= 60 ? "#e3b341" : "#f78166",fontWeight:600}}>{isUnder ? (100-h2hTR).toFixed(0) : h2hTR.toFixed(0)}%</span>{" "}<span style={{color:"#484f58"}}>of last {h2hTRGames}g H2H meetings.</span></>}
                          ...
                          return (
                            <div style={{background:"#0d1117",borderRadius:8,padding:"8px 10px",fontSize:11,color:"#8b949e",lineHeight:1.65}}>
                              <div>
                                ...
                                {play.softPct != null ? <>, and <span style={{color:softRateColor,fontWeight:600}}>{play.softPct}%</span> {play.hitterSoftLabel ?? "against weak pitching matchups"}{isPlatoonFallback && play.hitterSplitBA != null ? <span style={{color:"#484f58",fontSize:10}}> (hits .{Math.round(play.hitterSplitBA*1000).toString().padStart(3,"0")} vs {pitcherHand === "R" ? "RHP" : "LHP"})</span> : !isPlatoonFallback && play.softGames ? <span style={{color:"#484f58",fontSize:10}}> ({play.softGames}g)</span> : ""}</> : ""}.
