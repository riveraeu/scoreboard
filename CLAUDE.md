...
- `/api/auth/shadow-stats` — `GET`, admin (`Authorization: Bearer <ADMIN_KEY>`). Returns row counts from `shadow_plays` by date/category/dc. Add `?trigger=1` to also run the shadow-snapshot inline (uses `CRON_SECRET` internally, no external secret needed from caller). Handler: `api/lib/handlers/auth.js`.
