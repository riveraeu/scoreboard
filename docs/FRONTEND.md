...
Renders the `/api/shadow-report` payload (cron 6:00am PT, KV-cached 25h; `↻ Refresh` forces `?bust=1` regen). The prose `buildBriefing` blob and the truePct category-scoreboard table are gone; structure is now, top to bottom:
0. **DO THIS** (`DoThisBanner`, added 2026-06-21; ladder audit 2026-07-01) — renders ABOVE the title row: the single highest-priority action for the morning, picked by a fall-through ladder in `_doThisCandidates(d)` (candidates pushed per-tier, then SORTED by tier): (1) **data health** — fires only when `dataHealth.actionable` (catastrophic resolution `<0.5`, or the same coverage/CLV warning ≥2 days running — the server-side `shadow:healthstreak:{date}` KV counter escalates a streak into a live-pipeline-bug action; one-day dips stay caution-only); (2) **betting changes** — any `bettingBoard` row whose `doThis.action` ∈ {`Add to gate`, `Pull from gate`, `Look deeper`} (`_BET_ACTIONS`), grouped by action; (2.2) **model accuracy** — any `accuracyBoard` row in `_ACC_ACTIONS` (`Improve inputs` = market out-predicts → run tune:residual; `Recalibrate` = model beats price but miscalibrated → L2 de-shrink; `Diagnose then stop` = MARKET_SHARPER one-time check), suppressing the tune:residual nags for `INPUT_SEARCH_EXHAUSTED` cats and `Recalibrate` below `RECAL_MIN_N` (200) / while the skill trend is still moving. **Both exhaustion maps are DATED** (`{key: exhaustedAt}`), live in the shared `api/lib/model-holds.js` (2026-07-01 — imported by BOTH this page and the shadow-report brief builder, so the two surfaces agree on parked diagnostics) and auto-UN-suppress via `_stillExhausted` when the row's `formulaCutoff` (stamped on both boards) moves past the exhaustion date — a formula reset re-opens the search with no manual list edit; (2.5) **validate ripe** — ungated + `eligible` + n≥50 + STRENGTHENING → run tune:gate + `?brier=1` (eligibility required: the hard gate refuses ineligible cats regardless); (3) **build a NEXT-flagged roadmap market** (explicit "this first" override); (3.15) **derive per-category bet window** — eligible + accuracy-n≥200 + ROI outside [67,91], suppressed once the key lands in `CATEGORY_BET_WINDOWS` (done-detection off the config source of truth, same pattern as SERIES_CONFIG shipped-detection) or in dated `WINDOW_SEARCH_EXHAUSTED`; (3.2) **build the next merely-unshipped roadmap market** (harvesting accrued data outranks authoring new markets unless NEXT-flagged); (3.25) **vet shortlisted** / (3.5) **triage detected** markets; (3.6) **sportsbook GAP build**; (4) **`SCHEDULED_CHECKPOINTS`** — ALL dated follow-ups live in this one list (Polymarket 1b re-check, x* residual re-open, KXLMBGAME liquidity), each surfacing on its date and persisting until its entry is removed/re-dated — a "come back in ~2 weeks" wait must NEVER live only in a code comment or roadmap badge (those don't fire); (4.6) **sportsbook observatory floor**; (5) **all-clear floor** naming the next upcoming checkpoint (so the banner never just vanishes). The first tier is the PRIMARY (colored ▶ headline + one-line why); the full queue ships in the copy-button text.
...
3.5. **DAILY BRIEF** (`DailyBrief`, 2026-07-01) — renders `d.brief`, a **server-generated deterministic prose summary** built by `_buildDailyBrief` in `api/lib/handlers/shadow.js` (reader order since 2026-07-04: health — one dataHealth-pinned trust sentence with a clean/caution/action tone dot · headline "can we bet anything yet?" · changed-since-yesterday, diffed against yesterday's KV `shadow:report:{date}`, null when no prior report exists · model state, notable categories only, with a decay-caution clause when a positive-skill category's reliable learning trend ≤ −0.05 · betting accrual clocks — per eligible category the in-window ROI (flagged under n=30) + discovered window vs the n≥MIN_N_PROMOTE bar, gate PROMOTE/DEMOTE/HOLD lines lead · pricing/cross-venue findings from `sportsbookValidation` + `polymarketValidation` · takeaway — one closing line: actions if any, else the accrual clocks that matter). Every sentence is templated from numbers already in the payload — no free text — so the brief can't drift from the boards. Rows whose tune:residual diagnostic already ran empty (dated `INPUT_SEARCH_EXHAUSTED` in the shared `api/lib/model-holds.js`) render as *parked* rather than re-nagging "Diagnose then stop"/"Improve inputs" — the brief mirrors the banner's suppression exactly. **It replaces the always-visible accuracy grid**: `AccuracyBoard` now sits behind a collapsed "full grid" toggle (still the drill-down for calibration bands / tune:residual work), and `CrossVenueValidation` renders only when `brief` is absent. A stale pre-deploy report without `brief` falls back to the old always-visible grid until regen.
4. **MODEL ACCURACY + BETTING boards** (two-board split, 2026-06-22 — Layer 1 model quality vs Layer 2 bet selection). **`AccuracyBoard`** (`d.accuracyBoard`) — *do the models beat the price?* Scored ONLY on Brier-vs-market skill + truePct honesty (no price): per-row `verdict` (`BEATS_MARKET`/`TIE`/`MARKET_SHARPER`/`BUILDING`), `honest` action (Ship-eligible / No edge / Improve inputs / Accruing), Skill + model/market Brier, N, Honesty cell; sorted best-skill first; expand shows calibration bands + Brier line. **`BettingBoard`** (`d.bettingBoard`) — *what to bet*, led by `GateDigest` (one-line **GATE** digest counting `Add to gate`/`Pull from gate`/`Look deeper`; common day reads "No changes — keep betting N live categories", or the empty-gate posture). Carries the price-band validation ladder (verdict `PROMOTE`/`STRENGTHENING`/`HOLD`/`DEMOTE`/`NEGATIVE`/`BUILDING`, discovered window, ROI + 95% CI, n, `{n/CI/coh}` checklist) PLUS an `Eligible` column = the Layer-1 bridge (`eligible = accuracyVerdict===BEATS_MARKET`): **ineligible rows render greyed** with `doThis` = "Don't bet" (hard gate) — the window still shows so a tempting ROI visibly explains why it's unbettable. BUILDING rows collapse behind a "show" toggle; clicking a row expands its adaptive `priceBands` curve. The pre-split fused `ModelBoard` (one row, profit+honesty+skill in a single `doThis`) was replaced here.
