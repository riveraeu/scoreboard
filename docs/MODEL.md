...
- `mlb|hits` — re-added 2026-06-11; shipped with wrong series ticker `KXMLBHITS`, real ticker `KXMLBHIT` (fixed 2026-06-12, so shadow data starts 6/12). **FORMULA_CUTOFF bumped to 2026-06-30** (2026-07-19): pre-liquidity-gate dead-book YES≥80¢ rows (19 rows, hit 5.3% @ avg 86.7¢) faked the entire Brier skill (+0.053 → −0.005 without them) — same artifact class as totalBases 94¢; check `tune:gate` once post-6/30 n accrues.
...
- No frontend: not in liveStats — add live tracking only if the category is ever promoted.
