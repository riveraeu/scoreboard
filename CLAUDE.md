...
- `kTrendPts`: 0/1/2 — K-trend score (replaces `mlPts` in simScore formula). Shown in SimScore tooltip as "K-Trend". 2=trending up, 1=stable/null, 0=trending down. Also shown in explanation prose (play card + player card): 2 → green "K-rate trending ↑ — recent form favorable"; 0 → red "K-rate trending ↓ — recent form declining"; 1/null → silent. Replaces the ML sentence that previously appeared in prose.
