...
**Fetch fan-outs must be concurrency-capped (`pLimit`, 2026-07-05)**: unbounded `Promise.all` fetch loops in the tonight pipeline (player pinfo/gamelogs, pitcher gamelogs, play-by-play) exhausted the function instance's file descriptors on big slates — undici surfaces this as `TypeError: fetch failed` with `e.cause.code` `EMFILE`/`EBUSY`, and once exhausted EVERY outbound fetch in the run fails (ESPN and Upstash alike; single-fetch routes and the snapshot cron stay healthy — that asymmetry is the diagnostic). Fix: wide fetch loops go through `pLimit(n)` (`api/lib/utils.js`, n≈16–20), and per-key KV reads over lists use `cache.getMany(keys, type)` (chunked MGET in `makeCache`, 20 keys/chunk; feature-test `CACHE2.getMany` and fall back to parallel GETs). Any NEW fan-out over a per-player/per-game list must follow both idioms. Trigger history: the 7/03 capture-band widening (+~25% qualifying markets) pushed the burst over the FD ceiling; symptom was the dashboard's "No games scheduled" (all `gameTime:null`, 96% of gamelogs failed).
