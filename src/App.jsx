...
const WORKER = "https://scoreboard-ivory-xi.vercel.app/api";
...
// abbrs that exist in >1 sport — these need ?sport= param when linking to a non-default sport
const _multiSportAbbrs = (() => {
  const seen = {}; TEAM_DB.forEach(t => { seen[t.abbr] = (seen[t.abbr] || 0) + 1; });
  return new Set(Object.keys(seen).filter(a => seen[a] > 1));
})();
function teamUrl(abbr, sport) {
  const upper = abbr.toUpperCase();
  // default sport = first entry for this abbr (MLB > NBA > NHL by TEAM_DB order)
...
const MOCK_PLAYS = [
  // ── GOLD (≥93%) ────────────────────────────────────────────────────────────
  {
    ...
    lineupKPct:21.5, pitcherCSWPct:32.1, pitcherKPct:26.5, pitcherKBBPct:18.2, pitcherAvgPitches:94.3, pitcherRecentKPct:29.8, pitcherSeasonKPct:26.5, log5Avg:31.6, log5Pct:96.1, expectedKs:8.5, parkFactor:1.00,
    simScore:13, finalSimScore:13, kpctMeets:true, kpctPts:3, kbbMeets:true, kbbPts:2, lkpMeets:true, lkpPts:2, pitchesPts:2, mlPts:2, totalPts:2, kTrendPts:2,
  ...
  // ── PURPLE (≥85%) ──────────────────────────────────────────────────────────
  {
    ...
    lineupKPct:null, pitcherCSWPct:31.4, pitcherKPct:26.2, pitcherKBBPct:11.4, pitcherAvgPitches:88.0, pitcherRecentKPct:25.9, pitcherSeasonKPct:26.2, log5Avg:null, log5Pct:null, expectedKs:null, parkFactor:0.88,
    simScore:8, finalSimScore:8, kpctMeets:true, kpctPts:3, kbbMeets:false, kbbPts:0, lkpMeets:true, lkpPts:1, pitchesPts:2, mlPts:2, totalPts:1, kTrendPts:1,
  ...
  // ── BLUE (≥80%) ────────────────────────────────────────────────────────────
  {
    ...
    lineupKPct:22.1, pitcherCSWPct:30.8, pitcherKPct:25.9, pitcherKBBPct:14.1, pitcherAvgPitches:86.7, pitcherRecentKPct:26.4, pitcherSeasonKPct:25.9, log5Avg:27.5, log5Pct:73.2, expectedKs:7.4, parkFactor:0.96,
    simScore:11, finalSimScore:11, kpctMeets:true, kpctPts:3, kbbMeets:true, kbbPts:1, lkpMeets:true, lkpPts:2, pitchesPts:2, mlPts:2, totalPts:2, kTrendPts:1,
  ...
  // ── GREEN (≥75%) ───────────────────────────────────────────────────────────
  {
    ...
    lineupKPct:25.8, pitcherCSWPct:33.7, pitcherKPct:28.1, pitcherKBBPct:20.4, pitcherAvgPitches:96.0, pitcherRecentKPct:31.4, pitcherSeasonKPct:28.1, log5Avg:33.1, log5Pct:90.2, expectedKs:8.9, parkFactor:0.97,
    simScore:13, finalSimScore:13, kpctMeets:true, kpctPts:3, kbbMeets:true, kbbPts:2, lkpMeets:true, lkpPts:3, pitchesPts:2, mlPts:2, totalPts:1, kTrendPts:2,
    gameTotal:8.0, gameMoneyline:-150, pitcherHand:"R", gameDate:TODAY,
  },
  // ── WHITE (≥70%) ──────────────────────────────────────────────────────────
  {
    playerName:"Shohei Ohtani", playerId:39832, sport:"mlb", playerTeam:"LAD", position:"DH",
    opponent:"LAA", oppRank:5, oppMetricValue:4.92, oppMetricLabel:"ERA allowed", oppMetricUnit:"ERA",
    stat:"hrr", threshold:1, kalshiPct:78, americanOdds:-355,
    seasonPct:80.0, blendGames:20, pct25:null, pct25Games:0, pct26:80.0, pct26Games:20,
    softPct:92.9, softGames:28, truePct:88.5, edge:10.5,
    kelly:0.180, ev:0.072, historicalHitRate:80.0, historicalGames:20,
    hitterSoftLabel:"vs Jose Quintana",
    lineupKPct:null, pitcherKPct:null, gameDate:TODAY,
    hitterBa:0.281, hitterBaTier:"good",
    hitterSimScore:9, hitterFinalSimScore:9,
    hitterLineupSpot:1, hitterWhipMeets:false, hitterPlatoonPts:0, hitterParkMeets:true,
    hitterBarrelPct:26.3, hitterBarrelPts:3,
    hitterGameTotal:12, hitterTotalPts:2,
    pitcherWHIP:1.32, pitcherFIP:4.9,
    hitterSplitBA:0.229,
    oppPitcherHand:"L",
  ...
  // ── NHL (green ~78%) ─────────────────────────────────────────────────────
  {
    ...
    nhlSimScore:11, nhlPreSimScore:8, nhlOpportunity:21.3, nhlShotsAdj:2.4, nhlSaRank:7, isB2B:false,
  ...
  // ── NHL (white ~72%) ─────────────────────────────────────────────────────
  {
    ...
    nhlSimScore:8, nhlPreSimScore:5, nhlOpportunity:22.1, nhlShotsAdj:-0.8, nhlSaRank:22, isB2B:false,
