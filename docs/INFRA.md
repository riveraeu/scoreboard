...
| `0 13` | 6:00am report | 5:00am | `/api/shadow-report` | trails the `50 12` resolver by 10 min so yesterday is fully resolved → born complete. `INCOMPLETE_REPORT_TTL` (15min) still guards an early `?bust=1`. Runs BEFORE the `15 16` series scan, so `newMarkets` lags one cycle |
| `15 16` | 9:15am series scan | 8:15am | `/api/kalshi-series-scan` | feeds the NEXT day's `newMarkets` (report now runs earlier at `0 13`) |
