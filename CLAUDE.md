...
- `src/components/MatchupCard.jsx` — per-game card: team logos, center stats, NBA lineup in collapsible drawer (lazy-fetched on first open); umpire and weather data are still fetched and used in True% but no longer displayed in the card UI (removed Apr 2026); NBA injury badges (OUT red / GTD yellow) from `nbaMeta.injuries` always visible outside the drawer (two-column, home right-aligned); home team lineup right-aligned inside drawer to mirror header layout. **MLB cards show header only** — pitcher row, confirmed/expected lineup badge, and batting lineup drawer all removed (Apr 2026). **Center header** is state-aware: `pre` → game time + O/U + ML + spread; `in` → score (`awayScore – homeScore`) + inning/status in yellow; `post` → score + "Final" in gray. Score data comes from `game.gameState/gameDetail/homeScore/awayScore` (seeded by `mlbMeta.gameScores` via `buildGames`). **No play badge logic** — play cards are handled by `PlaysColumn` below the matchup grid. Props: `{ game, mlbMeta, mlbMetaTomorrow, nbaMeta, navigateToPlayer, navigateToTeam }`. **Date-aware meta selection**: `isToday` computed from `gameDate` vs PT date — used to select MLB game odds (`mlbGameOdds = isToday ? mlbMeta?.gameOdds : {}`); `mlbMetaTomorrow` passed through but only consumed for game odds, not pitcher/lineup display.
