...
0. **DO THIS** (`DoThisBanner`, added 2026-06-21) — renders ABOVE the title row: the single highest-priority action for the morning, picked by a fall-through ladder in `_doThisCandidates(d)`: (1) **data health** — fires only if `dataHealth.warnings.length` (a cron/coverage/resolution/CLV warning poisons everything below, so it trumps all); (2) **model changes** — any `modelBoard` row whose `doThis.action` ∈ {`Add to gate`, `Pull from gate`, `Tune down`, `Look deeper`} (`_CHANGE_ACTIONS`), grouped by action into the headline; (3) **build next market** — lowest-`rank` `MODEL_NEXT` entry (names its `NEXT` item for the infra row); (4) **expand to Polymarket** (static backlog floor). The first actionable tier is the PRIMARY (colored ▶ headline + one-line why); the rest render as a dimmed `then → …` queued ladder. Tiers 3–4 are always available, so a quiet healthy day reads "Build {next market}" with Polymarket queued. Pure frontend digest of data already on the page — no report-payload change.
