...
//                         null means "default to first qualified, else first available".
//                         App.jsx resets this whenever activeTab or direction changes.
...
//   - editPickId          The pick.id whose row is currently in edit mode (odds input),
//                         or null. Lives here because the MyPicks render branch needs it.
//   - gamelogSort         { col, dir } — column + direction for the per-game log table.
//   - expandedPlays       Set of play ids whose detail row is expanded inline in the
//                         lineups grid. Lives here because expansion is per-render state.
//   - chartMonth          "YYYY-MM" — which month the calibration / picks chart shows.
...
export function usePlayerCardState() {
  ...
  const [editPickId, setEditPickId] = React.useState(null);
