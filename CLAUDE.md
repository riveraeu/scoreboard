...
- **SimScore** (max 10, edge gate only — 5 stats × 2pts each):
  ...
  - H2H hit rate (`hitterH2HHitRatePts`): rate vs tonight's pitcher from gamelog (H2H dates only). **Requires ≥ 8 games for 2pts** (≥80% hit rate); 5–7 games caps at 1pt max (≥70% → 1pt, else 0pt); < 5 games → null → 1pt abstain. When platoon fallback was used (`_hrrUsingTeamFallback = true`), forced to 1pt abstain regardless of `_h2hVals` — no real H2H data exists. **No team fallback for SimScore** — pitcher H2H only.
