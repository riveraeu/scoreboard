...
### "Kalshi and ESPN disagree on a whole MLB series" — postponed games and makeup doubleheaders
**Symptom**: `/api/kalshi-dryrun-check` reports a burst of `disagree` rows all sharing one matchup, and the sampled tickers carry a date one day behind the row's `gameDate`. Found 2026-07-29 as 65 rows on CLE@CIN.

**What happened** (2026-07-27): the 7/27 CLE@CIN game was postponed and made up as a 7/28 doubleheader. Kalshi settled `KXMLB*-26JUL271910CLECIN` against makeup **game 1** (CLE 6–5). The ESPN path graded against **game 2** (CIN 2–0). **Kalshi was right; ESPN was wrong.** A disagreement never implies Kalshi erred — check the physical game before assuming.

The row set broke in two distinct ways, and both had to be fixed:
- **Rows dated 7/27** (ticker date == game_date) matched ESPN's 7/27 event, which was still listed as `STATUS_POSTPONED` — and **ESPN reports a postponed game as `state:"post"` with `completed:false` and 0-0 scores.** `_resolveRow` guarded only on `state !== "post"`, so it graded them as a 0–0 final: every `under` a win, every `over` a loss. This is the more dangerous of the two, because it is silent and applies to any postponement, not just ones with a doubleheader makeup.
- **Rows dated 7/28** (game_date re-attributed forward of the ticker date) were hydrated by `gameTimes`, which keys on `sport:team:ptDate` with **last-event-wins** — on a doubleheader date that hands the makeup market game 2's start time. `/api/live` then matched game 2 exactly and correctly, against the wrong event.

**The trap**: the ticker cannot identify the makeup game. `26JUL271910` encodes 19:10 ET, and game 2 also started 23:10Z — the same wall clock one day later — while the market settled against game 1 (17:40Z). Matching on ticker time picks game 2; matching on ticker date picks 7/27, when no game happened. MLB's "the makeup is game 1" convention is a habit, not a rule. **There is no ESPN-side mapping from a postponed market to its makeup game — only settlement knows.** Anything that guesses re-creates this bug in a new shape.

**Fix** (2026-07-29): detect, refuse to guess, defer to settlement.
- `/api/live` now passes `completed` through; `isNonFinalTerminal(game)` (`settlement-reconcile.js`) owns the `=== false` test — it must never be relaxed to falsiness, since `completed` is absent from payloads cached before this shipped and those must keep behaving as they did.
- `isMakeupReattributed(row)` flags `game_date > tickerDate`. **Strictly forward** — `game_date` is PT-derived and ticker dates are ET, so a legitimately late game can sit one day *behind* its ticker; only a forward gap means re-attribution.
- Flagged rows are excluded from the ESPN resolve loop entirely (not just from `won` — otherwise `reconcileGrades` would keep an `actualValue` read off the wrong game), and settlement is promoted from comparison-only to authoritative **for those rows only**. This is not a preference for settlement on a calibrated family; it is that ESPN has no answer to prefer, and the alternative is no grade at all.
- Counters `postponedMakeup.{makeupReattributed,postponedEvent,settlementGraded,voided,stillUngraded}` on the resolver response — these rows leave the disagreement tripwire by construction, so they need their own visibility or the fix trades a wrong grade for a blind spot.

**Repair for rows already written**: `/api/shadow-snapshot?regradepostponed=1` (ADMIN, `?dry=1` to report, `?days=N` window). The population is **structural, not "disagrees with settlement"** — it regrades every row of a matchup where some row was re-attributed forward, which catches both the original-date and makeup-date halves in one pass. Regrading on disagreement alone would hand settlement authority over the calibrated families wholesale, converting a repair into a policy change. `actual_value` is nulled alongside `won`, since it was read off the wrong physical game.
