...
- **SimScore** (max 10, edge gate only — 5 stats × 2pts each):
  ...
  - **Matchup Rate** (`hitterH2HHitRatePts`): BvP used only when `_pitcherVals.length >= 12` (~35+ PAs — mature signal); platoon-adjusted rate is the primary path for ~90% of matchups. **BvP path (≥12 games)**: ≥80% hit rate → 2pts (requires ≥8 games; 8–11 caps at 1pt max); ≥70% → 1pt; else → 0pt. **Platoon path** (`_hrrUsingTeamFallback = true`): full `hitterPlatoonPts` scale — advantage (ratio ≥1.10) → 2pts, neutral (≥1.00) → 1pt, disadvantage → 0pts. `MIN_H2H = 12`. `hitterPlatoonRatio` (exact ratio) and `hitterH2HSource` (`"bvp"|"platoon"|"abstain"`) included in all play/drop output for debugging.
