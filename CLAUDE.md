...
- **SimScore** (max 10, edge gate only — 5 stats × 2pts each):
  ...
  - **Matchup Rate** (`hitterH2HHitRatePts`): **BvP path** (≥10 games vs specific pitcher): ≥80% hit rate → 2pts, ≥70% → 1pt, else → 0pt. **Handedness path** (< 10 BvP games, pitcher hand known): cross-references all loaded `pitcherGamelogs` for same-handedness starters — finds dates each same-hand pitcher started vs `playerTeam`, filters batter's gamelog to those (date, oppTeam) pairs, computes actual HRR hit rate; ≥10 qualifying games → same ≥80%/≥70% tiers; overrides `softPct`/`softLabel` with real rate + "vs RHP"/"vs LHP". **Abstain** (< 10 games in either path) → 1pt. `hitterPlatoonRatio` still computed for display/debug only (not SimScore). `hitterH2HSource` (`"bvp"|"hand"|"abstain"`) in all play/drop output.
