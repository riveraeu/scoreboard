...
| `parseGameScores(events, normFn)` | Extract live/final scores from ESPN scoreboard events; `normFn` maps raw abbrs to canonical form (e.g. GS→GSW); returns `{homeTeam, awayTeam, state, detail, homeScore, awayScore, gameDate, gameTime}` keyed by home team abbr |
...
- `nbaMeta` — object in `/api/tonight` response: `{ gameOdds: {abbr: {ml, total, spread}}, injuries: {abbr: [{name, status}]}, gameScores: {abbr: {homeTeam, awayTeam, state, detail, homeScore, awayScore, gameDate, gameTime}} }`. `gameOdds` built from `sportByteam.nbaGameOdds` with short-code normalization (GS→GSW etc.). `injuries` from `nbaInjuryMap` (already built for C2 injury boost). `gameScores` from `parseGameScores(events, normFn)` applied to NBA ESPN scoreboard events. Used by `MatchupCard` to populate NBA center header (O/U / ML / spread) and seed live/final scores into `buildGames` so game cards show scores even after Kalshi markets settle.
- `nhlMeta` — object in `/api/tonight` response: `{ gameScores: {abbr: {...}}, gameOdds: {abbr: {ml, total, spread}} }`. `gameScores` from `parseGameScores` on NHL ESPN scoreboard. `gameOdds` from `sportByteam.nhlGameOdds` (already normalized via `normTeam("nhl", ...)`), transformed to `{ml, total, spread}` format. Used by `buildGames` (scores) and `MatchupCard` (total/ML/spread in center header).
