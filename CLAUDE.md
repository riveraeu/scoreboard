...
| URL routing, App.jsx state shape, ReportPage (Today / Model board / Discipline / Ops), live tracking, sizing, color doctrine | `docs/FRONTEND.md` |
...
| `/api/shadow-resolver` | `handlers/shadow.js` | Crons: 2am, 3:05am, 7am PT — resolves shadow plays via ESPN live scores (4-pass name lookup). SELECT uses pooled primary (`neonQuery {write:true}`; unpooled replica serves stale-empty on cold wake). `noData` rows are usually next-day Kalshi pre-listings (resolve T+1, not stuck). Detail → memories [neon http api], [resolver noData = pre-listings]. |
