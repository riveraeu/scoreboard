...
**Capture vs bet window**: `[KALSHI_GATE, KALSHI_CAP]` = the `qualified` *bet* flag, but shadow LOGGING uses a wider `[CAPTURE_GATE 55, CAPTURE_CAP 97]` band so calibration sees the full favorite curve and the bet window can be derived from data. Applied at the collection pre-filters in `tonight.js` and at the favorite-side selectors in every Phase-1 shadow module. `config.test.js` pins `CAPTURE ⊃ [KALSHI_GATE, KALSHI_CAP]`.
