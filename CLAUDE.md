...
| `/api/shadow-report` | `handlers/shadow.js` | Cron (9:30am PT) — generates morning briefing: today's top picks, `yesterdayRecap` (sourced from TRACKED account picks — Upstash `picks:{userId}`, most-active key when read via cron/admin — for games with `gameDate`=yesterday; W-L/pushes/pending, units-weighted `roi` = Σpl/Σunits where pl uses stored `units` $ stake + `kalshiAvgCents` entry, recommendation-based not fill-accurate; per-pick outcomes), category scoreboard, opportunity bands, CLV, optimal picks/day. **Decision-grade fields (2026-06-16):** `dataHealth` (coverage from KV `shadow:coverage:{date}` + yesterday resolution/CLV-capture), per-category **formula-window** floors (`FORMULA_CUTOFFS`) so calibration never mixes superseded-formula rows, **significance verdicts** (gate n≥50 / formula n≥200 + |Δ|>2·SE + band coherence), `perGameRoi`+`optimalPerGameCap`, and `disciplineFlags` (tracked-pick operational lessons — flagged-band/over-cap/neg-CLV; NOT a model correction). Cached in KV 25h. Auth: CRON_SECRET or ADMIN_KEY/JWT. `?bust=1` forces regen. |
...
| `/api/push/notify` | `handlers/push.js` | Crons: 15:08, 22:08, 17:03, 03:03 UTC (each ~3–8 min after a `tonight` cron) — reads `shadow:staging:{date}`, model-gates plays (`passesModelGate`: `dcQualified && edge≥5 && passesCategoryGate`), then decides market trust: cached-trusted plays pass as-is, cached-untrusted ones get a throttled **live-book** check (`liveBookTrusts` walks the FULL resting book for a $30 reference stake — promotes 0-traded-volume-but-deep markets, 2026-06-15). Dedups against KV `push:notified:{date}` (30h TTL, once/play/PT-day). `broadcastPersonalized` sends per-subscription with **logged-in ledger suppression** (a play already in a user's tracked picks is dropped from THAT user's payload; anonymous subs get everything); >3 personal fresh → one summary push, ≤3 → individual. Prunes 404/410 subs. Response carries `liveVerified` count. Auth: CRON_SECRET or ADMIN_KEY. |
