...
async function handleShadowResolver({ path, request, env, cache }) {
  ...
  // The SETTLEMENT_AUTHORITATIVE_SPORTS are excluded here because they are graded entirely off
  // Kalshi settlement now (Phase B, 2026-07-27) — their 15 per-sport ESPN resolver blocks and row
  // partitions were deleted, not disabled. DERIVED from settlement-reconcile.js rather than
  // re-listing the 14 sports: the two must agree exactly, and a sport that fell through to
  // teamRows would get ESPN team-matched, which is precisely what these families must not do (no
  // ESPN feed knows tennis players, soccer draws, or a 3-way league table). Adding a sport to
  // SETTLEMENT_AUTHORITATIVE_SPORTS now correctly excludes it here with no second edit.
  //
  // ESPN cross-checking is deliberately KEPT for the calibrated mlb/nba/wnba/nhl families — that
  // tripwire caught two real bugs in three days (the 2026-07-25 Kalshi mis-settlement and the
  // 2026-07-27 cross-day mis-grade) and is the only reason either was found.
  const teamRows = rows.filter(r => !isSettlementAuthoritative(r.sport));
  ...
  // `noData`/`skipped` are ESPN-feed metrics. This used to subtract `rec.espnAbsent`, because the
  // two populations OVERLAPPED: ESPN ran on the authoritative sports too, so a row could be
  // counted in `noData` and then rescued by settlement.
  //
  // After Phase B (2026-07-27) they are DISJOINT — `noData` can only come from `teamRows`, which
  // now excludes every settlement-authoritative sport, while `espnAbsent` counts only
  // settlement-graded (= authoritative) rows. Subtracting one from the other would remove rows
  // that were never in `noData`, silently clamping the result to 0 and reporting a clean number
  // regardless of the truth. `noData` is already net of settlement, so it stands on its own.
  // Kept as an explicit field rather than deleted so the response contract is unchanged.
  const noDataNetOfSettlement = noData;
