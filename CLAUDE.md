...
- **Polymarket comparison (game totals only)**: Uses `/events?series_id=X&closed=false&limit=50` (MLB=3, NBA=10345, NHL=10346) — NOT `/markets`. Game totals are nested child markets inside each event. Event titles use "Team A vs. Team B" format; teams resolved via `POLY_NAME_TO_ABBR` reverse map (full MLB names, short NBA/NHL nicknames). O/U line parsed from child market question text; Kalshi threshold = `ouLine + 0.5` (e.g. Poly "O/U 8.5" → thresh=9). Matches are threshold-specific so overlap only occurs when Kalshi qualifies at 70%+ on a line that Polymarket also carries — typically for higher alt lines (Kalshi qualifying at O8.5 matches Poly main line). `outcomePrices[overIdx]` gives implied probability (×100 = `polyPct`). Cached in Redis at `poly:totals:{date}` with 300s TTL; busted by `?bust=1`. Play output includes `polyPct`, `polyVol`, `bestVenue` ("kalshi"|"polymarket"), `bestEdge` (truePct − min(kalshi,poly)). UI: edge badge shows `bestEdge`; tooltip shows "Kalshi: Xc | Polymarket: Yc"; teal `[P]` pill appears when Polymarket wins. Market report totals table has a `Poly` column (teal = better than Kalshi, gray = worse/unavailable).
