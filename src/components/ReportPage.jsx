...
const MODEL_NEXT = [
  {
    ...
    markets: [
      ...
      { t: "infra", badge: "NEXT", ticker: "per-category price windows", title: "tune:residual on mlb|totalBases (n=255, 2026-06-29) found the bettable edge lives ABOVE the universal [67,91] cap — Brier skill +0.5 at price 85–95 / threshold 4–6 (favorites win even more than modeled), while the [67,91] bulk at 65–80 is a market tie and the 75–80 band is OVERconfident, so the edge gate ADVERSELY selects (model-edge anti-correlated with accuracy: skill +0.156 on the no-edge plays it skips vs −0.18 at edge 15+). Fix is a per-category price window (shift totalBases to ~[80,95]), NOT a de-shrink (calibration is non-monotonic) or a new input (no dim showed a trustworthy gradient). Build the per-category-window plumbing + validate on totalBases (the only model with volume in the high-skill band); mlb|hits (n=185) confirms the same shape, too thin to act. Un-parks the Phase-2 price-window work. [[project-percategory-price-window]]" },
