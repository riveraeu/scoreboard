...
**Currently allowed: NONE — the gate is EMPTY as of 2026-06-19** (all four below pulled after formula-clean validation). Re-enable a category only after `tune:gate` confirms a coherent +ROI band at n≥50 + non-negative Brier skill (`?brier=1`).
...
- **NBA / WNBA / NHL player props + MLB strikeouts**: `capWeight = 0.5` (max 50% anchor at 20+ games). Counters pure-sim drift from empirical hit rate at the threshold.
