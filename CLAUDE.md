...
- `tonight/{tennis-match,soccer,soccer-advance,golf-h2h,mlb-outs,nascar,fight,nba-summer,lmb-ml}.js` — the Phase-1 emit modules. Each groups its Kalshi series, emits the favorite side (Kalshi YES in window), and pushes into a **dedicated array** (e.g. `tennisPlays`, `soccerPlays`) merged into `shadow:staging` ONLY — never the client response. They bypass prop dedup / gameTime filter / card builder. **All 8 hardcoded `gameTime: null`, which silently blocked `computeMakerQuote`'s pre-game gate (found 2026-07-23) — none of them had ever actually been maker-quotable.** Fixed 2026-07-23 for 7 (tennis, soccer, fight, nba-summer, lmb-ml, mlb-outs — real gameTime now sourced from the same ESPN/statsapi/`gameTimes` map each module already reads; nascar via one added per-race fetch, see `nascar.js`/`getRaceGameTime`). `mlb-outs` reused the exact `gameTimes` lookup idiom `props.js` already uses (keyed off the pitcher's own team, tomorrow-date fallback), since `emitMlbOutsPlays` sits in the same tonight.js pipeline pass. `golf-h2h` deferred — its data source has no per-matchup tee-time granularity, only tournament/round-level dates. **Separately, all 8 also never set `kalshiTicker`** (a second maker-eligibility gap, found while scoping whether grading could use Kalshi's own settlement instead of ESPN) — fixed same day, one line per file (the ticker was already captured at parse time in every case, just never threaded to the push)
