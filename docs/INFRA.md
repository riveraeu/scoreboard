...
**Edge SWR (2026-06-15):** the normal (non-debug, non-bust) response sets `Cache-Control: public, s-maxage=120, stale-while-revalidate=600` (header plumbed via the new string-opts branch of `jsonResponse`). Snap-first assembly is ~7s even when snaps are warm, so the Vercel CDN now serves a cached copy fresh for 2 min — matching the snap cron + client poll cadence so the displayed set is no staler than the 2-min poll already allowed — then serves the instant stale copy for 10 more min while revalidating in the background. This removes the synchronous 7s wait from essentially every load during active hours. Safe because display-price staleness no longer carries placement risk: the order modal walks the live book at placement ([[project_orderbook_live_walk]]). `?bust=1` and `?debug=1` stay `no-store` (bust = manual fresh; debug feeds shadow-snapshot logging) so neither serves nor populates the shared CDN cache. **Tunable:** raise `s-maxage` (e.g. 300) for fewer recomputes at the cost of newly-qualified plays surfacing later.
