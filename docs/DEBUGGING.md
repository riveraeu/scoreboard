...
**A too-large multi-row INSERT killed quoting for two days** (2026-08-12/13). `maker_quotes` built one statement for the whole slate; once it grew past what Neon would accept, the insert threw, so no segments were written, so the next cycle rebuilt the same oversized batch — a deadlock that cannot self-recover. All bulk inserts now go through `neonBatchInsert` (`api/lib/neon.js`). **Use it; don't hand-roll** — the row count is usually the slate, which only grows.

Three things this cost, worth not repeating:

- **The binding limit is Neon's HTTP request size, not Postgres' 65535 bind parameters.** The first fix chunked to 60000 params — legal for Postgres — and failed identically. `PARAM_BUDGET` is **8000** (~200KB/chunk) on measurement, not theory, and a test pins it an order of magnitude below 65535 so a later "use the real limit" optimization fails loudly.
- **Neon reports every one of these as a bare `Database request failed`** — no `code`, no `detail`, no `routine`, and the string exists in neither the driver nor this repo. Don't expect PG error fields; they are genuinely absent.
- **Label every DB round-trip.** `updateMakerQuotes` makes four, and the telemetry recorded all four as one indistinguishable string. Adding `[insert(5698x15)]`-style labels localized it in a single cycle after three deploys of guessing. `neonBatchInsert` annotates with the **chunk's** size too, which is what separates "batch too big" from "chunking is broken".
