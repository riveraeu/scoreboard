                    ...
                    <div key={`${sport}|${stat}`} style={{marginBottom:18}}>
                      ...
                      {(() => {
                        ...
                        const XCOLS = {
                          ...
                          "mlb|totalRuns":    [{k:"sim",l:"Score"},{k:"combinedRPG",l:"Comb RPG"},{k:"umpire",l:"Umpire"},{k:"homeERA",l:"H ERA"},{k:"awayERA",l:"A ERA"},{k:"mlbOu",l:"O/U"}],
                          ...
                          "mlb|teamRuns":     [{k:"sim",l:"Score"},{k:"ttUmpire",l:"Umpire"},{k:"ttWhip",l:"WHIP"},{k:"ttL10RPG",l:"L10 RPG"},{k:"ttH2HHR",l:"H2H HR%"},{k:"ttOu",l:"O/U"},{k:"ttOpp",l:"Opp"}],
                        ...
                        const xcell = (m, k) => {
                          ...
                          // Total PPG columns
                          if (k==="combinedRPG") { const v = m.combinedRPG; return v != null ? <span style={{color:v>=10.5?"#3fb950":v>=9.0?"#e3b341":"#8b949e",fontWeight:600}}>{v.toFixed(1)}</span> : DASH; }
                          ...
                          if (k==="homeOffRtg"||k==="awayOffRtg") { const v = m[k]; return v != null ? <span style={{color:v>=118?"#3fb950":v>=113?"#e3b341":"#8b949e",fontWeight:600}}>{v.toFixed(1)}</span> : DASH; }
                          ...
                          if (k==="nbaTotPace") { const pa = m.projPace != null && m.leagueAvgPace != null ? parseFloat((m.projPace - m.leagueAvgPace).toFixed(1)) : null; if (pa == null) return DASH; const _pp = (m.homePace == null||m.awayPace==null||m.leagueAvgPace==null)?1:(m.homePace>m.leagueAvgPace+2&&m.awayPace>m.leagueAvgPace+2)?2:(m.homePace>m.leagueAvgPace||m.awayPace>m.leagueAvgPace)?1:0; return <span style={{color:_pp===2?"#3fb950":_pp===1?"#e3b341":"#8b949e",fontWeight:600}}>{(pa>0?"+":"")+pa}</span>; }
                          ...
                          if (k==="ttWhip") { const v=m.oppWHIP; if (v==null) return DASH; const color=v>1.35?"#3fb950":v>1.20?"#e3b341":"#8b949e"; return <span style={{color,fontWeight:600}}>{v.toFixed(2)}</span>; }
                          if (k==="ttL10RPG") { const v=m.teamL10RPG; if (v==null) return DASH; const color=v>5.0?"#3fb950":v>4.0?"#e3b341":"#8b949e"; return <span style={{color,fontWeight:600}}>{v.toFixed(1)}</span>; }
                        ...
                        const COL_TIPS = {
                          ...
                          umpire:"Umpire run factor (1/K-factor): loose zone = more walks/scoring; green ≥1.05 (2pts), yellow ≥0.97 (1pt), red <0.97 (tight zone)",
                          ...
                          ttUmpire:"Umpire run factor (1/K-factor) — loose zone = more baserunners = favors scoring. green ≥1.05 (2pts), yellow ≥0.97 (1pt), else 0pts; null = 1pt abstain. Hover for umpire name.",
