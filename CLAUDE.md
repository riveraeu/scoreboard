...
| `/api/sportsbook-deltas` | `shadow.js` | GET (ADMIN/JWT) — multi-day Kalshi-vs-sharp-book ML divergence. `meanSigned>0` = Kalshi systematically cheap = lag edge. `?minAbs=N` adds a `rows` tail dump (\|Δ\|≥N¢, LIMIT 200) for outlier audits; `?purge=all` (ADMIN) wipes for re-baseline. Populated only once `THE_ODDS_API_KEY` is provisioned |
