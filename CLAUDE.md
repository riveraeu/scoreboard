...
| `/api/push/notify` | `handlers/push.js` | Crons: 15:08, 22:08, 17:03, 03:03 UTC (each ~3–8 min after a `tonight` cron) — reads `shadow:staging:{date}`, filters to push-actionable plays (`dcQualified && edge≥5 && passesCategoryGate && !untrustedMarket`), dedups against KV `push:notified:{date}` (30h TTL, once/play/PT-day), broadcasts via `web-push`. >3 fresh → one summary push; ≤3 → individual. Prunes 404/410 subs. Auth: CRON_SECRET or ADMIN_KEY. |
