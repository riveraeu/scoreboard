...
- **Maker V2 sub-50 live trial — KILLED 2026-08-24, RE-ARMED 2026-08-25, EXPANDED 2026-08-26** — the sub-50 one-sided live trial (`docs/MAKER_V2_SUBFIFTY_TRIAL.md`) went live 2026-08-24 on `wnba|points` + `mlb|f5total|10-14` and was killed the same day (`POST /api/maker-v2-kill`) after real capital committed was found to be ~5x the intended combined $60 cap ($313 real vs. $60 nominal) — `groupExposureCents`/`costCents` (`api/lib/maker-live.js`) were computing the per-group cap off the SOLD side's price instead of the real `100 − price` cost of the complementary buy `sellAsBuy()` actually places. Fixed same day (both sites now use `100 − price`); re-armed 2026-08-25 with a per-cell `resumeFrom` so `wnba-points` gets a fresh ledger (its bug-era −$60.75 doesn't permanently halt it) while `mlb-f5total` keeps its track record. **Expanded 2026-08-26** with three diagnostic cells (`mlb-hrr-negctrl`, `wnba3p-killdiag`, `wnbasp-onesided` — not added for expected profit, see the doc's "Expansion" section + its fixed checkpoint criteria, `MAKER_V2_DIAGNOSTIC_CHECKPOINT` 2026-09-09) and a new portfolio-level `MAKER_V2_GLOBAL_CAP_CENTS` backstop across all groups combined. This does NOT reverse the original 80-84 favorite-band verdict (six dissolutions, no demonstrated edge) — `computeWantedMakerQuotes` no longer reads `MAKER_V2_BAND` at all. V1 quoting, nightly tape replay, and grading stay ON regardless. **Check `/api/maker-v2-board`'s `armed`/`groups[]` for live status** (`exposureCents`/`realizedCents`/`halted` per group).
...
| Sub-50 one-sided live trial (`wnba\|points`, `mlb\|f5total\|10-14`, + 3 diagnostic cells added 8/26) — mechanism, sizing, per-group caps/stop-losses, checkpoints | `docs/MAKER_V2_SUBFIFTY_TRIAL.md` — **LIVE**, re-armed 2026-08-25, expanded 2026-08-26 (`armed:true` confirmed via `/api/maker-v2-board`) |
