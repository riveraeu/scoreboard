...
**Model version toggle (`v1` / `v2`)**: Per-user localStorage flag (`scoreboard_modelVersion`) selects which model the frontend asks for. `useModelVersion()` in `src/lib/hooks.js` is the canonical accessor. App.jsx appends `&model=v2` to all `/api/tonight` URLs when v2 is active, and `trackPlay` stamps every pick with the active `modelVersion`. Server-side `/api/tonight` reads `params.get("model")`, coerces to `v1 | v2`, and echoes back in the response. **Phase A landed today: v1 and v2 produce identical play output** — the flag is wired but no behavior diverges yet. **Phase B (TBD)**: `SimScore-as-attribution refactor` — when `modelVersion === "v2"`, SimScore is purely display, does not influence `truePct`, and the SimScore ≥ 8 qualification gate is bypassed (qualification keeps Kalshi window + edge gate). To add a Phase B branch: locate the SimScore-influences-truePct site, wrap with `if (modelVersion === "v2") { ... v2 logic ... } else { ... v1 logic ... }`. `/api/auth/calibration` accepts `?modelVersion=v1|v2|all` (default `all`) and always returns a top-level `byModelVersion: { v1, v2 }` breakdown — picks tracked before the toggle have no field and are treated as v1.
