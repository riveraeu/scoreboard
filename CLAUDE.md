...
- **Maker V2 KILLED 2026-08-24, not re-armed** — the sub-50 one-sided live trial (`docs/MAKER_V2_SUBFIFTY_TRIAL.md`: `wnba|points` + `mlb|f5total|10-14`) went live 2026-08-24 and was killed the same day (`POST /api/maker-v2-kill`, `armed:false` confirmed) after real capital committed was found to be ~5x the intended combined $60 cap ($313 real vs. $60 nominal) — `groupExposureCents`/`costCents` (`api/lib/maker-live.js`) were computing the per-group $30 cap off the SOLD side's price instead of the real `100 − price` cost of the complementary buy `sellAsBuy()` actually places. Side/mechanism were correct throughout; only the cap arithmetic was wrong. Fixed same day (both sites now use `100 − price`), `maker-live.test.js` updated, 354/354 tests pass. Existing positions from before the kill remain open and settle naturally. `SHELVED = false` in `api/lib/maker-live.js` still (shelving is separate from arming) but `armed:false` — re-arming is a deliberate follow-up, see the Status section atop `docs/MAKER_V2_SUBFIFTY_TRIAL.md`. This does NOT reverse the original 80-84 favorite-band verdict (six dissolutions, no demonstrated edge) — `computeWantedMakerQuotes` no longer reads `MAKER_V2_BAND` at all. V1 quoting, nightly tape replay, and grading stay ON regardless. **Check `/api/maker-v2-board`'s `armed`/`groups[]` for live status** (`exposureCents`/`realizedCents`/`halted` per group).
