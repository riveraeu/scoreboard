...
| `0 13` | 6:00am report | 5:00am | `/api/shadow-report` | early auto-gen; pre-7am-resolver so yesterday often <90% resolved → 15min `INCOMPLETE_REPORT_TTL` self-heals after the `0 14` resolver. Now runs BEFORE the `15 16` series scan, so `newMarkets` lags one cycle |
