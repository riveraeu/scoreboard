...
- **Maker V2 sub-50 live trial — KILLED 2026-08-24, RE-ARMED 2026-08-25, EXPANDED then `wnba-points` HALTED 2026-08-26, +1 candidate cell + `wnba-points` RE-ARMED 2026-08-27, cell-matching bug FIXED 2026-08-31** — the sub-50 one-sided live trial (`docs/MAKER_V2_SUBFIFTY_TRIAL.md`) went live 2026-08-24 on `wnba|points` + `mlb|f5total|10-14` and was killed the same day (`POST /api/maker-v2-kill`) after real capital committed was found to be ~5x the intended combined $60 cap ($313 real vs. $60 nominal) — `groupExposureCents`/`costCents` (`api/lib/maker-live.js`) were computing the per-group cap off the SOLD side's price instead of the real `100 − price` cost of the complementary buy `sellAsBuy()` actually places. Fixed same day (both sites now use `100 − price`); re-armed 2026-08-25. **Expanded 2026-08-26** with three diagnostic cells (`mlb-hrr-negctrl`, `wnba3p-killdiag`, `wnbasp-onesided` — not added for expected profit, see the doc's "Expansion" section + its fixed checkpoint criteria, `MAKER_V2_DIAGNOSTIC_CHECKPOINT` 2026-09-09) and a portfolio-level `MAKER_V2_GLOBAL_CAP_CENTS` backstop. **Same day, a THIRD real-capital bug found + `wnba-points` HALTED** (see doc's "Incident" section): a real order that filled in two pieces on Kalshi's side got its first piece recorded, flipped to `'executed'`, and was silently dropped from all future fill-polling — the second piece (84% of the position) was never applied, understating `wnba-points`' tracked exposure by ~$16 and letting real exposure run ~29% over its $30 cap undetected. Fixed in `maker-live.js` (both fill-polling sites now re-check any not-fully-filled order regardless of status, summing every matching fill event via the pure `reconciledFilledCount` rather than one event overwriting the total); `wnba-points`' cells removed from `MAKER_V2_LIVE_CELLS` entirely pending re-arm verification, global cap re-tuned $75→$60 to stay binding at 4 groups. **2026-08-27: the fix was confirmed working** (the exact Johnson order that triggered the halt corrected 4→25 post-fix, cross-checked against `/api/kalshi-fills`) and three changes followed same day: (1) `wnbapts-1014` (`wnba|points|10-14`) added — the first cell added under the 2026-08-26 process change that is NOT diagnostic: it cleared both free gates (robustness bar + netting screen, same both-halves-positive shape as the category's 3 prior registrations) and went straight to a live $15-cap/$7.50-stop-loss cell, checkpoint 2026-09-10 (doc's "Addition 2026-08-27" section); (2) `wnba-points`' original bands (20-24, 25-29) **re-armed** at the same HALVED $15/$7.50 sizing (not restored to their pre-halt $30/$15 — this group's own bug is why the halving discipline exists), fresh ledger `MAKER_V2_WNBA_POINTS_REARM` (doc's "Re-arm 2026-08-27" section); (3) a full backlog re-scan of every category's whole-book netting found `wnba|rebounds|65-69` (REJECTED 8/21 as an ISLAND) has flipped to an inverted-MIRROR shape — its 50+ half excluding the candidate went −1.88¢/ct (8/21) → +2.80¢/ct (8/27) — added as `wnbareb-6569`, same halved sizing, checkpoint 2026-09-10 (doc's second "Addition 2026-08-27" section). Trial now at **8 groups**. This does NOT reverse the original 80-84 favorite-band verdict (six dissolutions, no demonstrated edge) — `computeWantedMakerQuotes` no longer reads `MAKER_V2_BAND` at all. V1 quoting, nightly tape replay, and grading stay ON regardless. **Check `/api/maker-v2-board`'s `armed`/`groups[]` for live status** (`exposureCents`/`realizedCents`/`halted` per group). **2026-08-31: `wnba-points` had placed ZERO real orders for its entire post-8/27 ledger — found + fixed (commit `00481b4`)**: `matchLiveCell` picked a `MAKER_V2_LIVE_CELLS` entry by sport+category alone (first array match), so once `wnbapts-1014` (`band [10,14]`) was added ahead of `wnba-points` (`[20,24]`/`[25,29]`) on 8/27, EVERY `wnba|points` row resolved to `wnbapts-1014`'s cell regardless of actual price — a 22-29¢ ask was checked against band `[10,14]`, failed, and was silently dropped every tick. Found while investigating why 5 of 7 live-trial groups had gone quiet (a daily-report follow-up, not a screenshot cross-check like the prior three bugs). Fixed: `computeWantedMakerQuotes` now tries every candidate cell sharing a row's sport+category (`candidateLiveCells`, new — returns ALL matches) and picks whichever one's band actually contains the market's ask via the existing `computeMakerQuote` gate, rather than trusting array order. `matchLiveCell` kept as an explicitly non-band-aware convenience for the (still common) single-cell-per-category case. 372/372 tests pass, 2 new — one reproducing the collision, one pinned directly against the real `MAKER_V2_LIVE_CELLS` config. **4th real-capital-adjacent bug in this trial's history** (after the 8/24 exposure-cap basis, 8/25 expiration duty-cycle, and 8/26 multi-piece-fill bugs) — this one silenced a group rather than overexposing it, so no capital was ever at risk, but it means `wnba-points`' 8/27 "re-arm" produced zero real evidence until this fix; its checkpoint read should be judged from 2026-08-31 forward, not 8/27.
...
| Sub-50 one-sided live trial (`mlb\|f5total\|10-14` + 3 diagnostic cells added 8/26 + `wnbapts-1014`/`wnbareb-6569` added 8/27; `wnba\|points` HALTED 8/26, RE-ARMED 8/27 at halved sizing) — mechanism, sizing, per-group caps/stop-losses, checkpoints | `docs/MAKER_V2_SUBFIFTY_TRIAL.md` — **LIVE** at 8 groups, re-armed 2026-08-25 + 2026-08-27, expanded 8/26 + 8/27 (`armed:true` confirmed via `/api/maker-v2-board`) |
