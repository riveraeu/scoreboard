...
**MLB closing-line snapshot (`mlbClosingOdds`)**: ESPN scoreboard returns empty `odds` once a game is in/post, so live odds maps would lose entries mid-day. Single global Redis key holds `{"home|away|gameDate": {home, away}}`. For each game in `gameScores`: `state==='pre'` writes the live odds to the snapshot (live odds map chosen by gameDate vs today's PT — today's games use `_mlbGameOdds`, tomorrow's use `_mlbGameOddsTomorrow`); `state==='in'|'post'` overlays the snapshot back so the matchup card displays the *closing* line. 36h TTL refreshes on each write; key is not date-prefixed so it survives midnight rollover. **If no snapshot exists for an in/post game** (e.g., the service was first deployed mid-game), live odds entries are **cleared** rather than left visible — Kalshi/ESPN in-progress prices look like real lines but are model-derived mid-game values, so we hide them rather than mislead.
