...
- `tonight/brasileirao-ml.js` / `tonight/nwsl-ml.js` / `tonight/chnsl-ml.js` / `tonight/ligamx-ml.js` — leagues 2-5 of the model-free maker set, same shape as `club-soccer-ml.js` — see `api/lib/{brasileirao,nwsl,chnsl,ligamx}.js` above. Each still needs its own `tonight.js` parse branch/array + `shadow.js` resolver block (not generalized — see `soccer-modelfree.js` note above)
