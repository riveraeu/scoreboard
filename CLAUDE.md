...
- **WNBA Moneyline + Spread (v1, 2026-05-20)** — `KXWNBAGAME` / `KXWNBASPREAD`. Reuses `simulateNBAJoint` with per-team σ=11 (matches WNBA total sim). Lambdas captured into `_wnbaMlContext` from the WNBA branch of the game-total loop. Same gates as NBA. WNBA-specific dc rules mirror NBA: `wnbaHomeHeavyInjury`/`wnbaAwayHeavyInjury` at usage-out ≥ 0.30. **No lineup-posted penalty** (same reason as NBA — WNBA starters not posted pre-game). Calibration buckets: `wnba|ml`, `wnba|spread`; tabs: `wnba-ml`, `wnba-spread`.
- **NHL Moneyline + Spread (v1, 2026-05-20)** — `KXNHLGAME` / `KXNHLSPREAD`. **Reuses `simulateMLBJoint` directly** (per-team Poisson is sport-agnostic — same independent Poisson draws whether the unit is "runs" or "goals"). Lambdas captured into `_nhlMlContext` from the NHL branch of the game-total loop. Same gates. **OT/SO approximation:** joint sim is regulation-only Poisson, so the ~22% of NHL games that go past regulation are treated as ties dropped from ML numerator (same as MLB extras). For spread, the OT winner gets +1 goal in reality (and shootout winner gets +1), slightly under-projecting cover probability on tight regulation finishes — acceptable v1 noise, revisit if calibration shows spread-on-favorite systematic miss. DC rules extend the existing `homeGoalieTeamFallback`/`awayGoalieTeamFallback` penalties to fire on `ml` and `spread` (previously total-only). No lineup penalty — NHL has no exposed pre-game lineup data. Calibration buckets: `nhl|ml`, `nhl|spread`; tabs: `nhl-ml`, `nhl-spread`.

---

## Key Gotchas

**Regime-aware lambda blend (2026-05-21)**: NBA / WNBA / NHL game totals and NBA team totals now blend each team's recency-weighted recent-score average into the rating-based expected. Exponential decay with 21-day half-life; cap 0.85 / sample-denom 8 (`_regimeBlendWeight` in `api/[...path].js` ~line 3337). Replaces the prior fixed 4% NBA playoff boost (`_PLAYOFF_OFF_BOOST`) — a team-specific data-driven signal catches playoff scoring shifts AND mid-season hot/cold streaks without a manually-tuned multiplier. Helpers `_recencyWeight` + `_recentTeamScoreMean(schedMap, sport, team)` are hoisted to shared scope so both the game-total block (uses `_gtScheduleMap`) and the team-total block (uses `_ttScheduleMap`) can call them. MLB is untouched (lambda is pitcher-driven, separate signal). Filter `trackedAt < 2026-05-21` from NBA/WNBA/NHL total + NBA teamTotal calibration. Emitted fields on play: `regimeBlendW`, `homeRecentMean`/`awayRecentMean` (or `scoringRecentMean` for team totals).
