...
0. **DO THIS** (`DoThisBanner`, added 2026-06-21) — renders ABOVE the title row: the single highest-priority action for the morning, picked by a fall-through ladder in `_doThisCandidates(d)`: (1) **data health** — fires only if `dataHealth.warnings.length` (a cron/coverage/resolution/CLV warning poisons everything below, so it trumps all); (2) **betting changes** — any `bettingBoard` row whose `doThis.action` ∈ {`Add to gate`, `Pull from gate`, `Look deeper`} (`_BET_ACTIONS`), grouped by action; (2.2) **model accuracy** — any `accuracyBoard` row in `_ACC_ACTIONS` (`Improve inputs` = market out-predicts → run tune:residual; `Recalibrate` = model beats price but miscalibrated → L2 de-shrink), suppressing `Improve inputs` for `INPUT_SEARCH_EXHAUSTED` cats and `Recalibrate` below `RECAL_MIN_N` (200) so an un-ripe reweight doesn't nag; (3) **build next market** — lowest-`rank` `MODEL_NEXT` entry (names its `NEXT` item for the infra row); (4) **Polymarket divergence** — DATE-GATED on `POLY_RECHECK` (the Phase-1b kill-gate is a dated ~2-week wait, not a daily decision, so the floor stays quiet until the re-check date and resurfaces then); (5) **all-clear floor** — if nothing above fires, a calm "Nothing to act on today" naming the next scheduled checkpoint (so the banner never just vanishes). The first actionable tier is the PRIMARY (colored ▶ headline + one-line why); the rest render as a dimmed `then → …` queued ladder. A quiet healthy day now reads "Nothing to act on today · next checkpoint Polymarket ~{POLY_RECHECK}" rather than a daily Polymarket nag. Pure frontend digest of data already on the page — no report-payload change.
