// api/lib/tonight/ligamx-ml.js
// emitLigaMxMlPlays — Liga MX game winner (3-way home/away/tie), Phase 1, model-free. Fifth
// model-free maker league, same shape as club-soccer-ml.js (MLS) / brasileirao-ml.js /
// nwsl-ml.js / chnsl-ml.js.
//
...
// module only needs a real gameTime (fetched from ESPN via api/lib/ligamx.js), the parsed
...
//
// Output goes into a dedicated `ligamxPlays` array (NOT the shared `plays` array) so these rows
// bypass prop/total dedup, the gameTime filter, and the frontend card builder. tonight.js
// merges ligamxPlays into the shadow:staging payload only; the client never sees them.
// `ligamx|game` is not in the category gate.

import { CAPTURE_GATE, CAPTURE_CAP } from "../config.js";
import { getLigaMxSchedule } from "../ligamx.js";

export async function emitLigaMxMlPlays(ctx) {
  const { ligamxMarkets, ligamxPlays, cutoffStr, cache, isBustCache } = ctx;
  if (!ligamxMarkets || ligamxMarkets.length === 0) return;

  // Group every priced side by event so one schedule lookup serves all of that game's sides.
  const events = new Map();
  for (const m of ligamxMarkets) {
  ...
  await Promise.all(dateStrs.map(async (dateStr) => {
    schedulesByDate.set(dateStr, await getLigaMxSchedule({ dateStr, cache, isBustCache }));
  }));
